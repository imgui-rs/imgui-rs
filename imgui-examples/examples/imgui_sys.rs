...
fn main() {
	...
	system.main_loop(move |_, ui| {
		...
		unsafe {
			...
			if let Some(previous_window) = igFindWindowByName(im_str!("Hello world").as_ptr()).as_ref() {
				if previous_window.Collapsed == false {
			...
			igBegin(im_str!("Hello from raw cimgui").as_ptr(), null_mut(), ImGuiWindowFlags_None);
			...
			igText(im_str!("Current Window Position : {},{}", current_window.Pos.x, current_window.Pos.y).as_ptr());
