...
fn example_3(ui: &Ui) {
    ...
    w.build(ui, || {
        ...
        ui.text("This button interprets the input value [1.0, 0.0, 0.0, 1.0] as HSVA (default):");
        ColorButton::new(im_str!("HSVA black"), [1.0, 0.0, 0.0, 1.0])
            .input_mode(ColorEditInputMode::HSV)
            .build(ui);

        ui.separator();
        ui.text("This button interprets the input value [0.0, 1.0, 1.0, 1.0] as HSV(A):");
        ColorButton::new(im_str!("HSVA red"), [0.0, 1.0, 1.0, 1.0])
            .input_mode(ColorEditInputMode::HSV)
            .build(ui);
    });
}
