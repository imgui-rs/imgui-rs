    ...
    ui.window("ImGui Demo")
        ...
        .build(|| {
            ...
            if ui.collapsing_header("Widgets").build() {
                ...
                ui.color_edit4("color 2", &mut state.col2).build();

                ui.tree_node("Multi-component Widgets").build(|| {
                    ui.input_float2("input float2", &mut state.vec2f).build();
                    ui.input_int2("input int2", &mut state.vec2i).build();
                    ui.spacing();

                    ui.input_float3("input float3", &mut state.vec3f).build();
                    ui.input_int3("input int3", &mut state.vec3i).build();
                    ui.spacing();
                });
            }
            if ui.collapsing_header("Popups & Modal windows").build() {
                ui.tree_node("Popups").build(|| {
                    ui.text_wrapped("When a popup is active, it inhibits interacting with windows that are behind the popup. Clicking outside the popup closes it.");
