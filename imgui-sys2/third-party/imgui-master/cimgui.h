...
// Windows) CIMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used
...
#endif // #if !defined(IMGUI_USE_STB_SPRINTF)&&(defined(__clang__)||
       // defined(__GNUC__))
#endif // #if !defined(IMGUI_USE_STB_SPRINTF)&&
...
typedef struct ImDrawChannel_t
    ImDrawChannel; // Temporary storage to output draw commands out of order,
                   // used by ImDrawListSplitter and ImDrawList::ChannelsSplit()
typedef struct ImDrawCmd_t
    ImDrawCmd; // A single draw command within a parent ImDrawList (generally
...
typedef struct ImDrawListSharedData_t
    ImDrawListSharedData; // Data shared among multiple draw lists (typically
                          // owned by parent ImGui context, but you may create
                          // one yourself)
typedef struct ImDrawListSplitter_t
    ImDrawListSplitter; // Helper to split a draw list into different layers
                        // which can be drawn into out of order, then flattened
                        // back.
typedef struct ImDrawVert_t
    ImDrawVert; // A single vertex (pos + uv + col = 20 bytes by default.
...
typedef struct ImFontAtlas_t
    ImFontAtlas; // Runtime data for multiple fonts, bake multiple fonts into a
                 // single texture, TTF/OTF font loader
typedef struct ImFontBuilderIO_t
    ImFontBuilderIO; // Opaque interface to a font builder (stb_truetype or
                     // FreeType).
...
typedef struct ImFontGlyph_t
    ImFontGlyph; // A single font glyph (code point + coordinates within in
                 // ImFontAtlas + offset)
...
typedef struct ImColor_t
    ImColor; // Helper functions to create a color that can be converted to
             // either u32 or float4 (*OBSOLETE* please avoid using)
typedef struct ImGuiContext_t
    ImGuiContext; // Dear ImGui context (opaque structure, unless including
                  // imgui_internal.h)
...
typedef struct ImGuiInputTextCallbackData_t
    ImGuiInputTextCallbackData; // Shared state of InputText() when using custom
                                // ImGuiInputTextCallback (rare/advanced use)
typedef struct ImGuiKeyData_t
    ImGuiKeyData; // Storage for ImGuiIO and IsKeyDown(), IsKeyPressed() etc
                  // functions.
...
typedef struct ImGuiPayload_t
    ImGuiPayload; // User data payload for drag and drop operations
typedef struct ImGuiPlatformImeData_t
    ImGuiPlatformImeData; // Platform IME data for io.SetPlatformImeDataFn()
                          // function.
typedef struct ImGuiSizeCallbackData_t
    ImGuiSizeCallbackData; // Callback data when using
                           // SetNextWindowSizeConstraints() (rare/advanced use)
...
typedef struct ImGuiTableSortSpecs_t
    ImGuiTableSortSpecs; // Sorting specifications for a table (often handling
                         // sort specs for a single column, occasionally more)
typedef struct ImGuiTableColumnSortSpecs_t
    ImGuiTableColumnSortSpecs; // Sorting specification for one column of a
                               // table
typedef struct ImGuiTextBuffer_t
    ImGuiTextBuffer; // Helper to hold and append into a text buffer (~string
                     // builder)
typedef struct ImGuiTextFilter_t
    ImGuiTextFilter; // Helper to parse and apply text filters (e.g.
                     // "aaaaa[,bbbbb][,ccccc]")
typedef struct ImGuiViewport_t
    ImGuiViewport; // A Platform Window (always only one in 'master' branch), in
                   // the future may represent Platform Monitor
...
//   symbols in comments.
typedef int ImGuiKey; // -> enum ImGuiKey              // Enum: A key identifier
                      // (ImGuiKey_XXX or ImGuiMod_XXX value)
typedef int
    ImGuiMouseSource;  // -> enum ImGuiMouseSource      // Enum; A mouse input
                       // source identifier (Mouse, TouchScreen, Pen)
...
#endif // ImWchar [configurable type: override in imconfig.h with '#define
...
CIMGUI_API ImGuiContext *
ImGui_CreateContext(ImFontAtlas *shared_font_atlas /* = NULL */);
CIMGUI_API void ImGui_DestroyContext(
...
// Main
CIMGUI_API ImGuiIO *
ImGui_GetIO(void); // access the IO structure (mouse/keyboard/gamepad inputs,
                   // time, various configuration options/flags)
CIMGUI_API ImGuiStyle *ImGui_GetStyle(
    void); // access the Style structure (colors, sizes). Always use
           // PushStyleCol(), PushStyleVar() to modify style mid-frame!
CIMGUI_API void
ImGui_NewFrame(void); // start a new Dear ImGui frame, you can submit any
                      // command from this point until Render()/EndFrame().
CIMGUI_API void ImGui_EndFrame(
    void); // ends the Dear ImGui frame. automatically called by Render(). If
...
CIMGUI_API void
ImGui_Render(void); // ends the Dear ImGui frame, finalize the draw data. You
                    // can then get call GetDrawData().
CIMGUI_API ImDrawData *
ImGui_GetDrawData(void); // valid after Render() and until the next call to
                         // NewFrame(). this is what you have to render.

// Demo, Debug, Information
CIMGUI_API void ImGui_ShowDemoWindow(
    bool *p_open /* = NULL */); // create Demo window. demonstrate most ImGui
                                // features. call this to learn about the
                                // library! try to make it always available in
                                // your application!
CIMGUI_API void ImGui_ShowMetricsWindow(
    bool *p_open /* = NULL */); // create Metrics/Debugger window. display Dear
                                // ImGui internals: windows, draw commands,
                                // various internal state, etc.
CIMGUI_API void ImGui_ShowDebugLogWindow(
    bool *p_open /* = NULL */); // create Debug Log window. display a simplified
                                // log of important dear imgui events.
CIMGUI_API void ImGui_ShowStackToolWindow(
    bool *p_open /* = NULL */); // create Stack Tool window. hover items with
                                // mouse to query information about the source
                                // of their unique ID.
CIMGUI_API void ImGui_ShowAboutWindow(
    bool
        *p_open /* = NULL */); // create About window. display Dear ImGui
                               // version, credits and build/system information.
CIMGUI_API void ImGui_ShowStyleEditor(
    ImGuiStyle *ref /* = NULL */); // add style editor block (not a window). you
                                   // can pass in a reference ImGuiStyle
                                   // structure to compare to, revert to and
                                   // save to (else it uses the default style)
CIMGUI_API bool ImGui_ShowStyleSelector(
    const char *label); // add style selector block (not a window), essentially
                        // a combo listing the default styles.
CIMGUI_API void ImGui_ShowFontSelector(
    const char *label); // add font selector block (not a window), essentially a
                        // combo listing the loaded fonts.
CIMGUI_API void ImGui_ShowUserGuide(
    void); // add basic help/info block (not a window): how to manipulate ImGui
           // as an end-user (mouse/keyboard controls).
CIMGUI_API const char *ImGui_GetVersion(
    void); // get the compiled version string e.g. "1.80 WIP" (essentially the
           // value for IMGUI_VERSION from the compiled version of imgui.cpp)

// Styles
CIMGUI_API void ImGui_StyleColorsDark(
    ImGuiStyle *dst /* = NULL */); // new, recommended style (default)
CIMGUI_API void ImGui_StyleColorsLight(
    ImGuiStyle
        *dst /* = NULL */); // best used with borders and a custom, thicker font
CIMGUI_API void
ImGui_StyleColorsClassic(ImGuiStyle *dst /* = NULL */); // classic imgui style
...
// "Debug".
CIMGUI_API bool ImGui_Begin(const char *name, bool *p_open /* = NULL */,
...
//    the corresponding BeginXXX function returned true. Begin and BeginChild
//    are the only odd ones out. Will be fixed in a future update.]
CIMGUI_API bool ImGui_BeginChild(const char *str_id,
                                 ImVec2 size /* = ImVec2(0, 0) */,
                                 bool border /* = false */,
                                 ImGuiWindowFlags flags /* = 0 */);
CIMGUI_API bool ImGui_BeginChildID(ImGuiID id, ImVec2 size /* = ImVec2(0, 0) */,
                                   bool border /* = false */,
                                   ImGuiWindowFlags flags /* = 0 */);
...
CIMGUI_API bool ImGui_IsWindowFocused(
    ImGuiFocusedFlags
        flags /* = 0 */); // is current window focused? or its root/child,
                          // depending on flags. see flags for options.
CIMGUI_API bool ImGui_IsWindowHovered(
    ImGuiHoveredFlags
        flags /* = 0 */); // is current window hovered (and typically: not
                          // blocked by a popup/modal)? see flags for options.
                          // NB: If you are trying to check whether your mouse
                          // should be dispatched to imgui or to your app, you
                          // should use the 'io.WantCaptureMouse' boolean for
                          // that! Please read the FAQ!
CIMGUI_API ImDrawList *
ImGui_GetWindowDrawList(void); // get draw list associated to the current
                               // window, to append your own drawing primitives
CIMGUI_API ImVec2
ImGui_GetWindowPos(void); // get current window position in screen space (note:
                          // it is unlikely you need to use this. Consider using
                          // current layout pos instead, GetScreenCursorPos())
CIMGUI_API ImVec2 ImGui_GetWindowSize(
    void); // get current window size (note: it is unlikely you need to use
           // this. Consider using GetScreenCursorPos() and e.g.
           // GetContentRegionAvail() instead)
CIMGUI_API float ImGui_GetWindowWidth(
    void); // get current window width (shortcut for GetWindowSize().x)
CIMGUI_API float ImGui_GetWindowHeight(
    void); // get current window height (shortcut for GetWindowSize().y)

// Window manipulation
// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX
// functions (after Begin).
CIMGUI_API void ImGui_SetNextWindowPos(
    ImVec2 pos, ImGuiCond cond /* = 0 */); // Implied pivot = ImVec2(0, 0)
CIMGUI_API void ImGui_SetNextWindowPosEx(
    ImVec2 pos, ImGuiCond cond /* = 0 */,
    ImVec2 pivot /* = ImVec2(0, 0) */); // set next window position. call before
                                        // Begin(). use pivot=(0.5f,0.5f) to
                                        // center on given point, etc.
CIMGUI_API void ImGui_SetNextWindowSize(
    ImVec2 size,
    ImGuiCond
        cond /* = 0 */); // set next window size. set axis to 0.0f to force an
...
CIMGUI_API void ImGui_SetNextWindowSizeConstraints(
    ...
    void *custom_callback_data /* = NULL */); // set next window size limits.
                                              // use -1,-1 on either X/Y axis to
                                              // preserve the current size.
                                              // Sizes will be rounded down. Use
                                              // callback to apply non-trivial
                                              // programmatic constraints.
CIMGUI_API void ImGui_SetNextWindowContentSize(
    ImVec2
...
CIMGUI_API void ImGui_SetNextWindowCollapsed(
    bool collapsed,
    ImGuiCond
        cond /* = 0 */); // set next window collapsed state. call before Begin()
CIMGUI_API void ImGui_SetNextWindowFocus(
    void); // set next window to be focused / top-most. call before Begin()
CIMGUI_API void ImGui_SetNextWindowScroll(
    ImVec2 scroll); // set next window scrolling value (use < 0.0f to not affect
                    // a given axis).
CIMGUI_API void ImGui_SetNextWindowBgAlpha(
...
CIMGUI_API void ImGui_SetWindowPos(
    ImVec2 pos,
    ImGuiCond cond /* = 0 */); // (not recommended) set current window position
                               // - call within Begin()/End(). prefer using
                               // SetNextWindowPos(), as this may incur tearing
                               // and side-effects.
CIMGUI_API void ImGui_SetWindowSize(
    ImVec2 size,
    ImGuiCond
        cond /* = 0 */); // (not recommended) set current window size - call
...
CIMGUI_API void ImGui_SetWindowCollapsed(
    bool collapsed,
    ImGuiCond cond /* = 0 */); // (not recommended) set current window collapsed
                               // state. prefer using SetNextWindowCollapsed().
CIMGUI_API void ImGui_SetWindowFocus(
    void); // (not recommended) set current window to be focused / top-most.
           // prefer using SetNextWindowFocus().
CIMGUI_API void ImGui_SetWindowFontScale(
    float scale); // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you
                  ...
                  // style.ScaleAllSizes().
CIMGUI_API void
ImGui_SetWindowPosStr(const char *name, ImVec2 pos,
                      ImGuiCond cond /* = 0 */); // set named window position.
CIMGUI_API void ImGui_SetWindowSizeStr(
    const char *name, ImVec2 size,
    ImGuiCond cond /* = 0 */); // set named window size. set axis to 0.0f to
                               // force an auto-fit on this axis.
CIMGUI_API void ImGui_SetWindowCollapsedStr(
    const char *name, bool collapsed,
    ImGuiCond cond /* = 0 */); // set named window collapsed state
CIMGUI_API void ImGui_SetWindowFocusStr(
    const char *name); // set named window to be focused / top-most. use NULL to
                       // remove focus.
...
// confusion)
CIMGUI_API ImVec2
ImGui_GetContentRegionAvail(void); // == GetContentRegionMax() - GetCursorPos()
CIMGUI_API ImVec2 ImGui_GetContentRegionMax(
    void); // current content boundaries (typically window boundaries including
           // scrolling, or current column boundaries), in windows coordinates
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMin(
    void); // content boundaries min for the full window (roughly (0,0)-Scroll),
           // in window coordinates
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMax(
    void); // content boundaries max for the full window (roughly
...
CIMGUI_API float
ImGui_GetScrollX(void); // get scrolling amount [0 .. GetScrollMaxX()]
CIMGUI_API float
ImGui_GetScrollY(void); // get scrolling amount [0 .. GetScrollMaxY()]
CIMGUI_API void
ImGui_SetScrollX(float scroll_x); // set scrolling amount [0 .. GetScrollMaxX()]
CIMGUI_API void
ImGui_SetScrollY(float scroll_y); // set scrolling amount [0 .. GetScrollMaxY()]
CIMGUI_API float
ImGui_GetScrollMaxX(void); // get maximum scrolling amount ~~ ContentSize.x -
                           // WindowSize.x - DecorationsSize.x
CIMGUI_API float
ImGui_GetScrollMaxY(void); // get maximum scrolling amount ~~ ContentSize.y -
                           // WindowSize.y - DecorationsSize.y
CIMGUI_API void ImGui_SetScrollHereX(
    float center_x_ratio /* = 0.5f */); // adjust scrolling amount to make
                                        // current cursor position visible.
                                        // center_x_ratio=0.0: left, 0.5:
                                        // center, 1.0: right. When using to
                                        // make a "default/current item"
                                        // visible, consider using
                                        // SetItemDefaultFocus() instead.
CIMGUI_API void ImGui_SetScrollHereY(
    float center_y_ratio /* = 0.5f */); // adjust scrolling amount to make
                                        // current cursor position visible.
                                        // center_y_ratio=0.0: top, 0.5:
                                        // center, 1.0: bottom. When using to
                                        // make a "default/current item"
                                        // visible, consider using
                                        // SetItemDefaultFocus() instead.
CIMGUI_API void ImGui_SetScrollFromPosX(
    float local_x,
    float center_x_ratio /* = 0.5f */); // adjust scrolling amount to make given
                                        // position visible. Generally
                                        // GetCursorStartPos() + offset to
                                        // compute a valid position.
CIMGUI_API void ImGui_SetScrollFromPosY(
    float local_y,
    float center_y_ratio /* = 0.5f */); // adjust scrolling amount to make given
                                        // position visible. Generally
                                        // GetCursorStartPos() + offset to
                                        // compute a valid position.
...
CIMGUI_API void
ImGui_PushFont(ImFont *font); // use NULL as a shortcut to push default font
CIMGUI_API void ImGui_PopFont(void);
CIMGUI_API void
ImGui_PushStyleColor(ImGuiCol idx,
...
CIMGUI_API void
ImGui_PushStyleVar(ImGuiStyleVar idx,
                   float val); // modify a style float variable. always use this
                               // if you modify the style after NewFrame().
CIMGUI_API void ImGui_PushStyleVarImVec2(
    ImGuiStyleVar idx,
    ImVec2 val); // modify a style ImVec2 variable. always use this if you
                 // modify the style after NewFrame().
CIMGUI_API void ImGui_PopStyleVar(void); // Implied count = 1
CIMGUI_API void ImGui_PopStyleVarEx(int count /* = 1 */);
CIMGUI_API void
ImGui_PushTabStop(bool tab_stop); // == tab stop enable. Allow focusing using
                                  // TAB/Shift-TAB, enabled by default but you
                                  // can disable it for certain widgets
CIMGUI_API void ImGui_PopTabStop(void);
CIMGUI_API void ImGui_PushButtonRepeat(
...
CIMGUI_API void ImGui_PushItemWidth(
...
CIMGUI_API void ImGui_SetNextItemWidth(
    float item_width); // set width of the _next_ common large "item+label"
                       // widget. >0.0f: width in pixels, <0.0f align xx pixels
                       // to the right of window (so -FLT_MIN always align width
                       // to the right side)
CIMGUI_API float
ImGui_CalcItemWidth(void); // width of item given pushed settings and current
                           // cursor position. NOT necessarily the width of last
                           // item unlike most 'Item' functions.
CIMGUI_API void ImGui_PushTextWrapPos(
    float wrap_local_pos_x /* = 0.0f */); // push word-wrapping position for
                                          // Text*() commands. < 0.0f: no
                                          // wrapping; 0.0f: wrap to end of
                                          // window (or column); > 0.0f: wrap at
                                          // 'wrap_pos_x' position in window
                                          // local space
...
CIMGUI_API float
ImGui_GetFontSize(void); // get current font size (= height in pixels) of
                         // current font with current scale applied
CIMGUI_API ImVec2 ImGui_GetFontTexUvWhitePixel(
    void); // get UV coordinate for a while pixel, useful to draw custom shapes
           // via the ImDrawList API
CIMGUI_API ImU32 ImGui_GetColorU32(ImGuiCol idx); // Implied alpha_mul = 1.0f
CIMGUI_API ImU32 ImGui_GetColorU32Ex(
    ImGuiCol idx,
    float alpha_mul /* = 1.0f */); // retrieve given style color with style
                                   // alpha applied and optional extra alpha
                                   // multiplier, packed as a 32-bit value
                                   // suitable for ImDrawList
CIMGUI_API ImU32 ImGui_GetColorU32ImVec4(
    ImVec4 col); // retrieve given color with style alpha applied, packed as a
                 // 32-bit value suitable for ImDrawList
CIMGUI_API ImU32 ImGui_GetColorU32ImU32(
    ImU32 col); // retrieve given color with style alpha applied, packed as a
                // 32-bit value suitable for ImDrawList
CIMGUI_API const ImVec4 *ImGui_GetStyleColorVec4(
    ImGuiCol
...
CIMGUI_API void ImGui_Separator(
    void); // separator, generally horizontal. inside a menu bar or in
           // horizontal layout mode, this becomes a vertical separator.
CIMGUI_API void
ImGui_SameLine(void); // Implied offset_from_start_x = 0.0f, spacing = -1.0f
CIMGUI_API void ImGui_SameLineEx(
    float offset_from_start_x /* = 0.0f */,
    float spacing /* = -1.0f */);    // call between widgets or groups to layout
                                     // them horizontally. X position given in
                                     // window coordinates.
CIMGUI_API void ImGui_NewLine(void); // undo a SameLine() or force a new line
                                     // when in a horizontal-layout context.
CIMGUI_API void ImGui_Spacing(void); // add vertical spacing.
CIMGUI_API void ImGui_Dummy(
    ImVec2 size); // add a dummy item of given size. unlike InvisibleButton(),
                  // Dummy() won't take the mouse click or be navigable into.
CIMGUI_API void ImGui_Indent(void); // Implied indent_w = 0.0f
CIMGUI_API void ImGui_IndentEx(
    float indent_w /* = 0.0f */); // move content position toward the right, by
                                  // indent_w, or style.IndentSpacing if
                                  // indent_w <= 0
CIMGUI_API void ImGui_Unindent(void); // Implied indent_w = 0.0f
CIMGUI_API void ImGui_UnindentEx(
...
CIMGUI_API void ImGui_EndGroup(
    void); // unlock horizontal starting position + capture the whole group
           // bounding box into one "item" (so you can use IsItemHovered() or
           // layout primitives such as SameLine() on whole group, etc.)
CIMGUI_API ImVec2
ImGui_GetCursorPos(void); // cursor position in window coordinates (relative to
                          // window position)
CIMGUI_API float ImGui_GetCursorPosX(
    void); //   (some functions are using window-relative coordinates, such as:
           //   GetCursorPos, GetCursorStartPos, GetContentRegionMax,
           //   GetWindowContentRegion* etc.
CIMGUI_API float
ImGui_GetCursorPosY(void); //    other functions such as GetCursorScreenPos or
                           //    everything in ImDrawList::
CIMGUI_API void ImGui_SetCursorPos(
    ImVec2 local_pos); //    are using the main, absolute coordinate system.
CIMGUI_API void
ImGui_SetCursorPosX(float local_x); //    GetWindowPos() + GetCursorPos() ==
                                    //    GetCursorScreenPos() etc.)
CIMGUI_API void ImGui_SetCursorPosY(float local_y); //
CIMGUI_API ImVec2
ImGui_GetCursorStartPos(void); // initial cursor position in window coordinates
CIMGUI_API ImVec2 ImGui_GetCursorScreenPos(
    void); // cursor position in absolute coordinates (useful to work with
           // ImDrawList API). generally top-left == GetMainViewport()->Pos ==
           // (0,0) in single viewport mode, and bottom-right ==
           // GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport
           // mode.
CIMGUI_API void
ImGui_SetCursorScreenPos(ImVec2 pos); // cursor position in absolute coordinates
CIMGUI_API void ImGui_AlignTextToFramePadding(
    void); // vertically align upcoming text baseline to FramePadding.y so that
...
CIMGUI_API float ImGui_GetTextLineHeightWithSpacing(
    void); // ~ FontSize + style.ItemSpacing.y (distance in pixels between 2
           // consecutive lines of text)
CIMGUI_API float
ImGui_GetFrameHeight(void); // ~ FontSize + style.FramePadding.y * 2
CIMGUI_API float ImGui_GetFrameHeightWithSpacing(
    void); // ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y
...
//   normally displayed.
CIMGUI_API void ImGui_PushID(
    const char *str_id); // push string into the ID stack (will hash string).
CIMGUI_API void ImGui_PushIDStr(
    const char *str_id_begin,
    const char
        *str_id_end); // push string into the ID stack (will hash string).
CIMGUI_API void ImGui_PushIDPtr(
    const void *ptr_id); // push pointer into the ID stack (will hash pointer).
CIMGUI_API void ImGui_PushIDInt(
    int int_id); // push integer into the ID stack (will hash integer).
CIMGUI_API void ImGui_PopID(void); // pop from the ID stack.
CIMGUI_API ImGuiID
ImGui_GetID(const char *str_id); // calculate unique ID (hash of whole ID stack
                                 // + given parameter). e.g. if you want to
                                 // query into ImGuiStorage yourself
CIMGUI_API ImGuiID ImGui_GetIDStr(const char *str_id_begin,
                                  const char *str_id_end);
CIMGUI_API ImGuiID ImGui_GetIDPtr(const void *ptr_id);

// Widgets: Text
CIMGUI_API void
ImGui_TextUnformatted(const char *text); // Implied text_end = NULL
CIMGUI_API void ImGui_TextUnformattedEx(
    const char *text,
    const char
        *text_end /* = NULL */); // raw text without formatting. Roughly
                                 // equivalent to Text("%s", text) but: A)
                                 // doesn't require null terminated string if
                                 // 'text_end' is specified, B) it's faster, no
                                 // memory copy is done, no buffer size limits,
                                 // recommended for long chunks of text.
CIMGUI_API void ImGui_Text(const char *fmt, ...)
...
CIMGUI_API void ImGui_TextColored(ImVec4 col, const char *fmt, ...)
    IM_FMTARGS(2); // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt,
                   // ...); PopStyleColor();
CIMGUI_API void ImGui_TextColoredV(ImVec4 col, const char *fmt, va_list args)
    IM_FMTLIST(2);
CIMGUI_API void ImGui_TextDisabled(const char *fmt, ...)
    IM_FMTARGS(1); // shortcut for PushStyleColor(ImGuiCol_Text,
                   // style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...);
                   // PopStyleColor();
CIMGUI_API void ImGui_TextDisabledV(const char *fmt, va_list args)
    IM_FMTLIST(1);
CIMGUI_API void ImGui_TextWrapped(const char *fmt, ...) IM_FMTARGS(
...
CIMGUI_API void ImGui_LabelText(const char *label, const char *fmt, ...)
    IM_FMTARGS(
        2); // display text+label aligned the same way as value+label widgets
CIMGUI_API void ImGui_LabelTextV(const char *label, const char *fmt,
                                 va_list args) IM_FMTLIST(2);
CIMGUI_API void ImGui_BulletText(const char *fmt, ...)
    IM_FMTARGS(1); // shortcut for Bullet()+Text()
CIMGUI_API void ImGui_BulletTextV(const char *fmt, va_list args) IM_FMTLIST(1);
CIMGUI_API void ImGui_SeparatorText(
...
CIMGUI_API bool ImGui_ButtonEx(const char *label,
                               ImVec2 size /* = ImVec2(0, 0) */); // button
CIMGUI_API bool ImGui_SmallButton(
    const char
        *label); // button with FramePadding=(0,0) to easily embed within text
CIMGUI_API bool ImGui_InvisibleButton(
    const char *str_id, ImVec2 size,
    ImGuiButtonFlags
        flags /* = 0 */); // flexible button behavior without the visuals,
                          // frequently useful to build custom behaviors using
                          // the public api (along with IsItemActive,
                          // IsItemHovered, etc.)
CIMGUI_API bool
ImGui_ArrowButton(const char *str_id,
                  ImGuiDir dir); // square button with an arrow shape
CIMGUI_API bool ImGui_Checkbox(const char *label, bool *v);
CIMGUI_API bool ImGui_CheckboxFlagsIntPtr(const char *label, int *flags,
                                          int flags_value);
CIMGUI_API bool ImGui_CheckboxFlagsUintPtr(const char *label,
                                           unsigned int *flags,
                                           unsigned int flags_value);
CIMGUI_API bool
ImGui_RadioButton(const char *label,
                  bool active); // use with e.g. if (RadioButton("one",
                                // my_value==1)) { my_value = 1; }
CIMGUI_API bool
ImGui_RadioButtonIntPtr(const char *label, int *v,
                        int v_button); // shortcut to handle the above pattern
                                       // when value is an integer
CIMGUI_API void ImGui_ProgressBar(float fraction,
                                  ImVec2 size_arg /* = ImVec2(-FLT_MIN, 0) */,
                                  const char *overlay /* = NULL */);
CIMGUI_API void
ImGui_Bullet(void); // draw a small circle + keep the cursor on the same line.
...
CIMGUI_API void ImGui_Image(
    ImTextureID user_texture_id,
    ImVec2 size); // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), tint_col =
                  // ImVec4(1, 1, 1, 1), border_col = ImVec4(0, 0, 0, 0)
CIMGUI_API void ImGui_ImageEx(ImTextureID user_texture_id, ImVec2 size,
                              ImVec2 uv0 /* = ImVec2(0, 0) */,
                              ImVec2 uv1 /* = ImVec2(1, 1) */,
                              ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */,
                              ImVec4 border_col /* = ImVec4(0, 0, 0, 0) */);
CIMGUI_API bool ImGui_ImageButton(
    const char *str_id, ImTextureID user_texture_id,
    ImVec2 size); // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col =
                  // ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
CIMGUI_API bool ImGui_ImageButtonEx(const char *str_id,
                                    ImTextureID user_texture_id, ImVec2 size,
                                    ImVec2 uv0 /* = ImVec2(0, 0) */,
                                    ImVec2 uv1 /* = ImVec2(1, 1) */,
...
// created.
CIMGUI_API bool ImGui_BeginCombo(const char *label, const char *preview_value,
                                 ImGuiComboFlags flags /* = 0 */);
CIMGUI_API void
ImGui_EndCombo(void); // only call EndCombo() if BeginCombo() returns true!
CIMGUI_API bool
ImGui_ComboChar(const char *label, int *current_item, const char *const items[],
                int items_count); // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGui_ComboCharEx(const char *label, int *current_item,
                                  const char *const items[], int items_count,
                                  int popup_max_height_in_items /* = -1 */);
CIMGUI_API bool ImGui_Combo(
    const char *label, int *current_item,
    const char
        *items_separated_by_zeros); // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGui_ComboEx(
    const char *label, int *current_item, const char *items_separated_by_zeros,
    int popup_max_height_in_items /* = -1 */); // Separate items with \0 within
                                               // a string, end item-list with
                                               // \0\0. e.g. "One\0Two\0Three\0"
CIMGUI_API bool ImGui_ComboCallback(
    const char *label, int *current_item,
    bool (*items_getter)(void *data, int idx, const char **out_text),
    void *data, int items_count); // Implied popup_max_height_in_items = -1
CIMGUI_API bool ImGui_ComboCallbackEx(
    const char *label, int *current_item,
    bool (*items_getter)(void *data, int idx, const char **out_text),
...
CIMGUI_API bool
ImGui_DragFloat(const char *label,
                float *v); // Implied v_speed = 1.0f, v_min = 0.0f, v_max =
                           // 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGui_DragFloatEx(
    const char *label, float *v, float v_speed /* = 1.0f */,
    float v_min /* = 0.0f */, float v_max /* = 0.0f */,
    const char *format /* = "%.3f" */,
    ImGuiSliderFlags flags /* = 0 */); // If v_min >= v_max we have no bound
CIMGUI_API bool
ImGui_DragFloat2(const char *label,
                 float v[2]); // Implied v_speed = 1.0f, v_min = 0.0f, v_max =
                              // 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGui_DragFloat2Ex(const char *label, float v[2],
                                   float v_speed /* = 1.0f */,
                                   float v_min /* = 0.0f */,
                                   float v_max /* = 0.0f */,
                                   const char *format /* = "%.3f" */,
...
CIMGUI_API bool
ImGui_DragFloat3(const char *label,
                 float v[3]); // Implied v_speed = 1.0f, v_min = 0.0f, v_max =
                              // 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGui_DragFloat3Ex(const char *label, float v[3],
                                   float v_speed /* = 1.0f */,
                                   float v_min /* = 0.0f */,
                                   float v_max /* = 0.0f */,
                                   const char *format /* = "%.3f" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_DragFloat4(const char *label,
                 float v[4]); // Implied v_speed = 1.0f, v_min = 0.0f, v_max =
                              // 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGui_DragFloat4Ex(const char *label, float v[4],
                                   float v_speed /* = 1.0f */,
                                   float v_min /* = 0.0f */,
                                   float v_max /* = 0.0f */,
                                   const char *format /* = "%.3f" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_DragFloatRange2(
    const char *label, float *v_current_min,
    float
        *v_current_max); // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f,
                         // format = "%.3f", format_max = NULL, flags = 0
CIMGUI_API bool ImGui_DragFloatRange2Ex(
...
CIMGUI_API bool ImGui_DragInt(const char *label,
                              int *v); // Implied v_speed = 1.0f, v_min = 0,
                                       // v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGui_DragIntEx(
    const char *label, int *v, float v_speed /* = 1.0f */, int v_min /* = 0 */,
    int v_max /* = 0 */, const char *format /* = "%d" */,
    ImGuiSliderFlags flags /* = 0 */); // If v_min >= v_max we have no bound
CIMGUI_API bool ImGui_DragInt2(const char *label,
                               int v[2]); // Implied v_speed = 1.0f, v_min = 0,
                                          // v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGui_DragInt2Ex(const char *label, int v[2],
                                 float v_speed /* = 1.0f */,
                                 int v_min /* = 0 */, int v_max /* = 0 */,
                                 const char *format /* = "%d" */,
                                 ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_DragInt3(const char *label,
                               int v[3]); // Implied v_speed = 1.0f, v_min = 0,
                                          // v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGui_DragInt3Ex(const char *label, int v[3],
                                 float v_speed /* = 1.0f */,
                                 int v_min /* = 0 */, int v_max /* = 0 */,
                                 const char *format /* = "%d" */,
                                 ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_DragInt4(const char *label,
                               int v[4]); // Implied v_speed = 1.0f, v_min = 0,
                                          // v_max = 0, format = "%d", flags = 0
CIMGUI_API bool ImGui_DragInt4Ex(const char *label, int v[4],
                                 float v_speed /* = 1.0f */,
                                 int v_min /* = 0 */, int v_max /* = 0 */,
                                 const char *format /* = "%d" */,
                                 ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_DragIntRange2(
    const char *label, int *v_current_min,
    int *v_current_max); // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format
                         // = "%d", format_max = NULL, flags = 0
CIMGUI_API bool ImGui_DragIntRange2Ex(const char *label, int *v_current_min,
                                      int *v_current_max,
                                      float v_speed /* = 1.0f */,
                                      int v_min /* = 0 */, int v_max /* = 0 */,
                                      const char *format /* = "%d" */,
                                      ...
                                      ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_DragScalar(const char *label, ImGuiDataType data_type,
                 void *p_data); // Implied v_speed = 1.0f, p_min = NULL, p_max =
                                // NULL, format = NULL, flags = 0
CIMGUI_API bool ImGui_DragScalarEx(const char *label, ImGuiDataType data_type,
                                   ...
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_DragScalarN(const char *label, ImGuiDataType data_type, void *p_data,
                  int components); // Implied v_speed = 1.0f, p_min = NULL,
                                   // p_max = NULL, format = NULL, flags = 0
CIMGUI_API bool ImGui_DragScalarNEx(const char *label, ImGuiDataType data_type,
...
//   https://github.com/ocornut/imgui/issues/3361
CIMGUI_API bool
ImGui_SliderFloat(const char *label, float *v, float v_min,
                  float v_max); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_SliderFloatEx(
    const char *label, float *v, float v_min, float v_max,
    const char *format /* = "%.3f" */,
    ImGuiSliderFlags
        flags /* = 0 */); // adjust format to decorate the value with a prefix
                          // or a suffix for in-slider labels or unit display.
CIMGUI_API bool
ImGui_SliderFloat2(const char *label, float v[2], float v_min,
                   float v_max); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_SliderFloat2Ex(const char *label, float v[2], float v_min,
                                     float v_max,
                                     const char *format /* = "%.3f" */,
                                     ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_SliderFloat3(const char *label, float v[3], float v_min,
                   float v_max); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_SliderFloat3Ex(const char *label, float v[3], float v_min,
                                     float v_max,
                                     const char *format /* = "%.3f" */,
                                     ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_SliderFloat4(const char *label, float v[4], float v_min,
                   float v_max); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_SliderFloat4Ex(const char *label, float v[4], float v_min,
                                     float v_max,
                                     const char *format /* = "%.3f" */,
                                     ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_SliderAngle(
    const char *label,
    float *v_rad); // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f,
                   // format = "%.0f deg", flags = 0
CIMGUI_API bool ImGui_SliderAngleEx(const char *label, float *v_rad,
                                    float v_degrees_min /* = -360.0f */,
                                    float v_degrees_max /* = +360.0f */,
                                    const char *format /* = "%.0f deg" */,
                                    ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_SliderInt(const char *label, int *v, int v_min,
                                int v_max); // Implied format = "%d", flags = 0
CIMGUI_API bool ImGui_SliderIntEx(const char *label, int *v, int v_min,
                                  int v_max, const char *format /* = "%d" */,
                                  ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_SliderInt2(const char *label, int v[2], int v_min,
                                 int v_max); // Implied format = "%d", flags = 0
CIMGUI_API bool ImGui_SliderInt2Ex(const char *label, int v[2], int v_min,
                                   int v_max, const char *format /* = "%d" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_SliderInt3(const char *label, int v[3], int v_min,
                                 int v_max); // Implied format = "%d", flags = 0
CIMGUI_API bool ImGui_SliderInt3Ex(const char *label, int v[3], int v_min,
                                   int v_max, const char *format /* = "%d" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_SliderInt4(const char *label, int v[4], int v_min,
                                 int v_max); // Implied format = "%d", flags = 0
CIMGUI_API bool ImGui_SliderInt4Ex(const char *label, int v[4], int v_min,
                                   int v_max, const char *format /* = "%d" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_SliderScalar(const char *label, ImGuiDataType data_type, void *p_data,
                   const void *p_min,
                   const void *p_max); // Implied format = NULL, flags = 0
CIMGUI_API bool ImGui_SliderScalarEx(const char *label, ImGuiDataType data_type,
                                     void *p_data, const void *p_min,
                                     const void *p_max,
                                     const char *format /* = NULL */,
                                     ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_SliderScalarN(const char *label, ImGuiDataType data_type, void *p_data,
                    int components, const void *p_min,
                    const void *p_max); // Implied format = NULL, flags = 0
CIMGUI_API bool ImGui_SliderScalarNEx(const char *label,
                                      ...
                                      ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_VSliderFloat(const char *label, ImVec2 size, float *v, float v_min,
                   float v_max); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_VSliderFloatEx(const char *label, ImVec2 size, float *v,
                                     float v_min, float v_max,
                                     const char *format /* = "%.3f" */,
                                     ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool ImGui_VSliderInt(const char *label, ImVec2 size, int *v,
                                 int v_min,
                                 int v_max); // Implied format = "%d", flags = 0
CIMGUI_API bool ImGui_VSliderIntEx(const char *label, ImVec2 size, int *v,
                                   int v_min, int v_max,
                                   const char *format /* = "%d" */,
                                   ImGuiSliderFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_VSliderScalar(const char *label, ImVec2 size, ImGuiDataType data_type,
                    void *p_data, const void *p_min,
                    const void *p_max); // Implied format = NULL, flags = 0
CIMGUI_API bool ImGui_VSliderScalarEx(const char *label, ImVec2 size,
                                      ImGuiDataType data_type, void *p_data,
...
CIMGUI_API bool
ImGui_InputText(const char *label, char *buf, size_t buf_size,
                ImGuiInputTextFlags flags /* = 0 */); // Implied callback =
                                                      // NULL, user_data = NULL
CIMGUI_API bool ImGui_InputTextEx(const char *label, char *buf, size_t buf_size,
                                  ImGuiInputTextFlags flags /* = 0 */,
                                  ImGuiInputTextCallback callback /* = NULL */,
                                  void *user_data /* = NULL */);
CIMGUI_API bool ImGui_InputTextMultiline(
    const char *label, char *buf,
    size_t buf_size); // Implied size = ImVec2(0, 0), flags = 0, callback =
                      // NULL, user_data = NULL
CIMGUI_API bool ImGui_InputTextMultilineEx(
    const char *label, char *buf, size_t buf_size,
    ImVec2 size /* = ImVec2(0, 0) */, ImGuiInputTextFlags flags /* = 0 */,
    ImGuiInputTextCallback callback /* = NULL */, void *user_data /* = NULL */);
CIMGUI_API bool ImGui_InputTextWithHint(
    const char *label, const char *hint, char *buf, size_t buf_size,
    ImGuiInputTextFlags
        flags /* = 0 */); // Implied callback = NULL, user_data = NULL
CIMGUI_API bool
ImGui_InputTextWithHintEx(const char *label, const char *hint, char *buf,
                          size_t buf_size, ImGuiInputTextFlags flags /* = 0 */,
                          ImGuiInputTextCallback callback /* = NULL */,
                          void *user_data /* = NULL */);
CIMGUI_API bool ImGui_InputFloat(const char *label,
                                 float *v); // Implied step = 0.0f, step_fast =
                                            // 0.0f, format = "%.3f", flags = 0
CIMGUI_API bool ImGui_InputFloatEx(const char *label, float *v,
                                   float step /* = 0.0f */,
                                   float step_fast /* = 0.0f */,
                                   const char *format /* = "%.3f" */,
                                   ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputFloat2(const char *label,
                  float v[2]); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_InputFloat2Ex(const char *label, float v[2],
                                    const char *format /* = "%.3f" */,
                                    ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputFloat3(const char *label,
                  float v[3]); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_InputFloat3Ex(const char *label, float v[3],
                                    const char *format /* = "%.3f" */,
                                    ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputFloat4(const char *label,
                  float v[4]); // Implied format = "%.3f", flags = 0
CIMGUI_API bool ImGui_InputFloat4Ex(const char *label, float v[4],
                                    const char *format /* = "%.3f" */,
                                    ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputInt(const char *label,
               int *v); // Implied step = 1, step_fast = 100, flags = 0
CIMGUI_API bool ImGui_InputIntEx(const char *label, int *v, int step /* = 1 */,
                                 int step_fast /* = 100 */,
                                 ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGui_InputInt2(const char *label, int v[2],
                                ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGui_InputInt3(const char *label, int v[3],
                                ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGui_InputInt4(const char *label, int v[4],
                                ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool ImGui_InputDouble(const char *label,
                                  double *v); // Implied step = 0.0, step_fast =
                                              // 0.0, format = "%.6f", flags = 0
CIMGUI_API bool ImGui_InputDoubleEx(const char *label, double *v,
                                    double step /* = 0.0 */,
                                    double step_fast /* = 0.0 */,
                                    const char *format /* = "%.6f" */,
                                    ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputScalar(const char *label, ImGuiDataType data_type,
                  void *p_data); // Implied p_step = NULL, p_step_fast = NULL,
                                 // format = NULL, flags = 0
CIMGUI_API bool ImGui_InputScalarEx(const char *label, ImGuiDataType data_type,
                                    void *p_data,
                                    ...
                                    ImGuiInputTextFlags flags /* = 0 */);
CIMGUI_API bool
ImGui_InputScalarN(const char *label, ImGuiDataType data_type, void *p_data,
                   int components); // Implied p_step = NULL, p_step_fast =
                                    // NULL, format = NULL, flags = 0
CIMGUI_API bool ImGui_InputScalarNEx(const char *label, ImGuiDataType data_type,
                                     void *p_data, int components,
                                     const void *p_step /* = NULL */,
                                     const void *p_step_fast /* = NULL */,
                                     const char *format /* = NULL */,
...
CIMGUI_API bool ImGui_ColorEdit3(const char *label, float col[3],
                                 ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGui_ColorEdit4(const char *label, float col[4],
                                 ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGui_ColorPicker3(const char *label, float col[3],
                                   ImGuiColorEditFlags flags /* = 0 */);
CIMGUI_API bool ImGui_ColorPicker4(const char *label, float col[4],
                                   ImGuiColorEditFlags flags /* = 0 */,
                                   const float *ref_col /* = NULL */);
CIMGUI_API bool ImGui_ColorButton(
    const char *desc_id, ImVec4 col,
    ImGuiColorEditFlags flags /* = 0 */); // Implied size = ImVec2(0, 0)
CIMGUI_API bool ImGui_ColorButtonEx(
    const char *desc_id, ImVec4 col, ImGuiColorEditFlags flags /* = 0 */,
    ImVec2 size /* = ImVec2(0, 0) */); // display a color square/button, hover
                                       // for details, return true when pressed.
CIMGUI_API void ImGui_SetColorEditOptions(
    ImGuiColorEditFlags
...
CIMGUI_API bool
ImGui_TreeNodeStr(const char *str_id, const char *fmt, ...) IM_FMTARGS(
...
CIMGUI_API bool ImGui_TreeNodePtr(const void *ptr_id, const char *fmt, ...)
    IM_FMTARGS(2); // "
CIMGUI_API bool ImGui_TreeNodeV(const char *str_id, const char *fmt,
                                va_list args) IM_FMTLIST(2);
CIMGUI_API bool ImGui_TreeNodeVPtr(const void *ptr_id, const char *fmt,
                                   va_list args) IM_FMTLIST(2);
CIMGUI_API bool ImGui_TreeNodeEx(const char *label,
                                 ImGuiTreeNodeFlags flags /* = 0 */);
CIMGUI_API bool ImGui_TreeNodeExStr(const char *str_id,
                                    ImGuiTreeNodeFlags flags, const char *fmt,
                                    ...) IM_FMTARGS(3);
CIMGUI_API bool ImGui_TreeNodeExPtr(const void *ptr_id,
                                    ImGuiTreeNodeFlags flags, const char *fmt,
                                    ...) IM_FMTARGS(3);
CIMGUI_API bool ImGui_TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags,
                                  const char *fmt, va_list args) IM_FMTLIST(3);
CIMGUI_API bool ImGui_TreeNodeExVPtr(const void *ptr_id,
                                     ImGuiTreeNodeFlags flags, const char *fmt,
                                     va_list args) IM_FMTLIST(3);
CIMGUI_API void ImGui_TreePush(
...
CIMGUI_API float ImGui_GetTreeNodeToLabelSpacing(
    void); // horizontal distance preceding label when using TreeNode*() or
           // Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular
           // unframed TreeNode
CIMGUI_API bool ImGui_CollapsingHeader(
    const char *label,
    ImGuiTreeNodeFlags
        flags /* = 0 */); // if returning 'true' the header is open. doesn't
                          // indent nor push on ID stack. user doesn't have to
                          // call TreePop().
CIMGUI_API bool ImGui_CollapsingHeaderBoolPtr(
    const char *label, bool *p_visible,
    ImGuiTreeNodeFlags
        flags /* = 0 */); // when 'p_visible != NULL': if '*p_visible==true'
                          // display an additional small close button on upper
                          // right of the header which will set the bool to
                          // false when clicked, if '*p_visible==false' don't
                          // display the header.
CIMGUI_API void ImGui_SetNextItemOpen(
    bool is_open,
...
CIMGUI_API bool ImGui_Selectable(
    const char
        *label); // Implied selected = false, flags = 0, size = ImVec2(0, 0)
CIMGUI_API bool ImGui_SelectableEx(
    const char *label, bool selected /* = false */,
    ImGuiSelectableFlags flags /* = 0 */,
    ImVec2
        size /* = ImVec2(0, 0) */); // "bool selected" carry the selection state
                                    // (read-only). Selectable() is clicked is
                                    // returns true so you can modify your
                                    // selection state. size.x==0.0: use
                                    // remaining width, size.x>0.0: specify
                                    // width. size.y==0.0: use label height,
                                    // size.y>0.0: specify height
CIMGUI_API bool ImGui_SelectableBoolPtr(
    const char *label, bool *p_selected,
    ImGuiSelectableFlags flags /* = 0 */); // Implied size = ImVec2(0, 0)
CIMGUI_API bool ImGui_SelectableBoolPtrEx(
    const char *label, bool *p_selected, ImGuiSelectableFlags flags /* = 0 */,
    ImVec2 size /* = ImVec2(0, 0) */); // "bool* p_selected" point to the
...
// fit ~7 items
CIMGUI_API bool ImGui_BeginListBox(
    const char *label,
    ImVec2 size /* = ImVec2(0, 0) */); // open a framed scrolling region
CIMGUI_API void ImGui_EndListBox(
    void); // only call EndListBox() if BeginListBox() returned true!
CIMGUI_API bool ImGui_ListBox(const char *label, int *current_item,
                              const char *const items[], int items_count,
                              int height_in_items /* = -1 */);
CIMGUI_API bool ImGui_ListBoxCallback(
    const char *label, int *current_item,
    bool (*items_getter)(void *data, int idx, const char **out_text),
    void *data, int items_count); // Implied height_in_items = -1
CIMGUI_API bool ImGui_ListBoxCallbackEx(
    const char *label, int *current_item,
    bool (*items_getter)(void *data, int idx, const char **out_text),
...
// better!
CIMGUI_API void ImGui_PlotLines(
    const char *label, const float *values,
    int values_count); // Implied values_offset = 0, overlay_text = NULL,
                       // scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size =
                       // ImVec2(0, 0), stride = sizeof(float)
CIMGUI_API void ImGui_PlotLinesEx(const char *label, const float *values,
...
CIMGUI_API bool ImGui_BeginMenuBar(
    void); // append to menu-bar of current window (requires
           // ImGuiWindowFlags_MenuBar flag set on parent window).
CIMGUI_API void ImGui_EndMenuBar(
    void); // only call EndMenuBar() if BeginMenuBar() returns true!
CIMGUI_API bool
ImGui_BeginMainMenuBar(void); // create and append to a full screen menu-bar.
CIMGUI_API void ImGui_EndMainMenuBar(
    void); // only call EndMainMenuBar() if BeginMainMenuBar() returns true!
CIMGUI_API bool ImGui_BeginMenu(const char *label); // Implied enabled = true
CIMGUI_API bool ImGui_BeginMenuEx(
    const char *label,
    bool enabled /* = true */); // create a sub-menu entry. only call EndMenu()
                                // if this returns true!
CIMGUI_API void
ImGui_EndMenu(void); // only call EndMenu() if BeginMenu() returns true!
CIMGUI_API bool ImGui_MenuItem(
    const char
        *label); // Implied shortcut = NULL, selected = false, enabled = true
CIMGUI_API bool
ImGui_MenuItemEx(const char *label, const char *shortcut /* = NULL */,
                 bool selected /* = false */,
                 bool enabled /* = true */); // return true when activated.
CIMGUI_API bool ImGui_MenuItemBoolPtr(
...
CIMGUI_API void
ImGui_EndTooltip(void); // only call EndTooltip() if
                        // BeginTooltip()/BeginItemTooltip() returns true!
CIMGUI_API void ImGui_SetTooltip(const char *fmt, ...) IM_FMTARGS(
...
CIMGUI_API bool ImGui_BeginItemTooltip(
    void); // begin/append a tooltip window if preceding item was hovered.
CIMGUI_API void ImGui_SetItemTooltip(const char *fmt, ...)
    IM_FMTARGS(1); // set a text-only tooltip if preceeding item was hovered.
                   // override any previous call to SetTooltip().
CIMGUI_API void ImGui_SetItemTooltipV(const char *fmt, va_list args)
    IM_FMTLIST(1);
...
CIMGUI_API bool ImGui_BeginPopup(
    const char *str_id,
    ImGuiWindowFlags flags /* = 0 */); // return true if the popup is open, and
                                       // you can start outputting to it.
CIMGUI_API bool ImGui_BeginPopupModal(
    const char *name, bool *p_open /* = NULL */,
    ImGuiWindowFlags flags /* = 0 */); // return true if the modal is open, and
                                       // you can start outputting to it.
CIMGUI_API void
ImGui_EndPopup(void); // only call EndPopup() if BeginPopupXXX() returns true!
...
CIMGUI_API void ImGui_OpenPopup(
    const char *str_id,
    ImGuiPopupFlags popup_flags /* = 0 */); // call to mark popup as open (don't
                                            // call every frame!).
CIMGUI_API void ImGui_OpenPopupID(
    ImGuiID id,
    ImGuiPopupFlags popup_flags /* = 0 */); // id overload to facilitate calling
                                            // from nested stacks
CIMGUI_API void ImGui_OpenPopupOnItemClick(
    const char *str_id /* = NULL */,
    ImGuiPopupFlags
        popup_flags /* = 1 */); // helper to open popup when clicked on last
                                // item. Default to
                                // ImGuiPopupFlags_MouseButtonRight == 1. (note:
                                // actually triggers on the mouse _released_
                                // event to be consistent with popup behaviors)
CIMGUI_API void ImGui_CloseCurrentPopup(
    void); // manually close the popup we have begin-ed into.
...
CIMGUI_API bool
ImGui_BeginPopupContextItem(void); // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGui_BeginPopupContextItemEx(
    const char *str_id /* = NULL */,
    ImGuiPopupFlags
        popup_flags /* = 1 */); // open+begin popup when clicked on last item.
                                // Use str_id==NULL to associate the popup to
                                // previous item. If you want to use that on a
                                // non-interactive item such as Text() you need
                                // to pass in an explicit ID here. read comments
                                // in .cpp!
CIMGUI_API bool
ImGui_BeginPopupContextWindow(void); // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGui_BeginPopupContextWindowEx(
    const char *str_id /* = NULL */,
    ImGuiPopupFlags popup_flags /* = 1 */); // open+begin popup when clicked on
                                            // current window.
CIMGUI_API bool
ImGui_BeginPopupContextVoid(void); // Implied str_id = NULL, popup_flags = 1
CIMGUI_API bool ImGui_BeginPopupContextVoidEx(
    const char *str_id /* = NULL */,
    ImGuiPopupFlags
        popup_flags /* = 1 */); // open+begin popup when clicked in void (where
...
CIMGUI_API bool ImGui_IsPopupOpen(
    const char *str_id,
...
//        --------------------------------------------------------------------------------------------------------
// - 5. Call EndTable()
CIMGUI_API bool ImGui_BeginTable(
    const char *str_id, int column,
    ImGuiTableFlags flags /* = 0 */); // Implied outer_size = ImVec2(0.0f,
                                      // 0.0f), inner_width = 0.0f
CIMGUI_API bool ImGui_BeginTableEx(const char *str_id, int column,
                                   ImGuiTableFlags flags /* = 0 */,
                                   ImVec2 outer_size /* = ImVec2(0.0f, 0.0f) */,
                                   float inner_width /* = 0.0f */);
CIMGUI_API void
ImGui_EndTable(void); // only call EndTable() if BeginTable() returns true!
CIMGUI_API void
ImGui_TableNextRow(void); // Implied row_flags = 0, min_row_height = 0.0f
CIMGUI_API void ImGui_TableNextRowEx(
    ImGuiTableRowFlags row_flags /* = 0 */,
    float min_row_height /* = 0.0f */); // append into the first cell of a new
                                        // row.
CIMGUI_API bool ImGui_TableNextColumn(
    void); // append into the next column (or first column of next row if
           // currently in last column). Return true when column is visible.
CIMGUI_API bool
ImGui_TableSetColumnIndex(int column_n); // append into the specified column.
...
// scrolled.
CIMGUI_API void ImGui_TableSetupColumn(
    const char *label,
    ImGuiTableColumnFlags
        flags /* = 0 */); // Implied init_width_or_weight = 0.0f, user_id = 0
CIMGUI_API void ImGui_TableSetupColumnEx(
    const char *label, ImGuiTableColumnFlags flags /* = 0 */,
    float init_width_or_weight /* = 0.0f */, ImGuiID user_id /* = 0 */);
CIMGUI_API void ImGui_TableSetupScrollFreeze(
    int cols,
    int rows); // lock columns/rows so they stay visible when scrolled.
CIMGUI_API void
ImGui_TableHeadersRow(void); // submit all headers cells based on data provided
                             // to TableSetupColumn() + submit context menu
CIMGUI_API void ImGui_TableHeader(
...
CIMGUI_API ImGuiTableSortSpecs *ImGui_TableGetSortSpecs(
    void); // get latest sort specs for the table (NULL if not sorting).
           // Lifetime: don't hold on this pointer over multiple frames or past
           // any subsequent call to BeginTable().
CIMGUI_API int ImGui_TableGetColumnCount(
    void); // return number of columns (value passed to BeginTable)
...
CIMGUI_API const char *ImGui_TableGetColumnName(
    int column_n /* = -1 */); // return "" if column didn't have a name declared
                              // by TableSetupColumn(). Pass -1 to use current
                              // column.
...
CIMGUI_API void ImGui_TableSetColumnEnabled(
    int column_n,
...
CIMGUI_API void ImGui_TableSetBgColor(
...
CIMGUI_API void
ImGui_Columns(void); // Implied count = 1, id = NULL, border = true
CIMGUI_API void ImGui_ColumnsEx(int count /* = 1 */,
                                const char *id /* = NULL */,
                                bool border /* = true */);
CIMGUI_API void
ImGui_NextColumn(void); // next column, defaults to current row or next row if
                        // the current row is finished
CIMGUI_API int ImGui_GetColumnIndex(void); // get current column index
CIMGUI_API float ImGui_GetColumnWidth(
    int column_index /* = -1 */); // get column width (in pixels). pass -1 to
                                  // use current column
CIMGUI_API void ImGui_SetColumnWidth(
    int column_index,
    float width); // set column width (in pixels). pass -1 to use current column
CIMGUI_API float ImGui_GetColumnOffset(
    int column_index /* = -1 */); // get position of column line (in pixels,
                                  // from the left side of the contents region).
                                  // pass -1 to use current column, otherwise
                                  // 0..GetColumnsCount() inclusive. column 0 is
                                  // typically 0.0f
CIMGUI_API void ImGui_SetColumnOffset(
    int column_index,
    float
...
CIMGUI_API bool ImGui_BeginTabBar(
    const char *str_id,
    ImGuiTabBarFlags flags /* = 0 */); // create and append into a TabBar
CIMGUI_API void
ImGui_EndTabBar(void); // only call EndTabBar() if BeginTabBar() returns true!
CIMGUI_API bool ImGui_BeginTabItem(
    const char *label, bool *p_open /* = NULL */,
    ImGuiTabItemFlags
        flags /* = 0 */); // create a Tab. Returns true if the Tab is selected.
CIMGUI_API void ImGui_EndTabItem(
    void); // only call EndTabItem() if BeginTabItem() returns true!
CIMGUI_API bool ImGui_TabItemButton(
    const char *label,
    ImGuiTabItemFlags
        flags /* = 0 */); // create a Tab behaving like a button. return true
                          // when clicked. cannot be selected in the tab bar.
CIMGUI_API void ImGui_SetTabItemClosed(
    const char
...
CIMGUI_API void
ImGui_LogToTTY(int auto_open_depth /* = -1 */); // start logging to tty (stdout)
CIMGUI_API void
ImGui_LogToFile(int auto_open_depth /* = -1 */,
                const char *filename /* = NULL */); // start logging to file
CIMGUI_API void ImGui_LogToClipboard(
    int auto_open_depth /* = -1 */);   // start logging to OS clipboard
CIMGUI_API void ImGui_LogFinish(void); // stop logging (close file, etc.)
CIMGUI_API void ImGui_LogButtons(
    void); // helper to display buttons for logging to tty/file/clipboard
CIMGUI_API void ImGui_LogText(const char *fmt, ...)
...
CIMGUI_API bool ImGui_BeginDragDropSource(
    ImGuiDragDropFlags
        flags /* = 0 */); // call after submitting an item which may be dragged.
                          // when this return true, you can call
                          // SetDragDropPayload() + EndDragDropSource()
CIMGUI_API bool ImGui_SetDragDropPayload(
    const char *type, const void *data, size_t sz,
    ImGuiCond cond /* = 0 */); // type is a user defined string of maximum 32
                               // characters. Strings starting with '_' are
                               // reserved for dear imgui internal types. Data
                               // is copied and held by imgui. Return true when
                               // payload has been accepted.
CIMGUI_API void
ImGui_EndDragDropSource(void); // only call EndDragDropSource() if
                               // BeginDragDropSource() returns true!
CIMGUI_API bool ImGui_BeginDragDropTarget(
    void); // call after submitting an item that may receive a payload. If this
           // returns true, you can call AcceptDragDropPayload() +
           // EndDragDropTarget()
CIMGUI_API const ImGuiPayload *ImGui_AcceptDragDropPayload(
    const char *type,
...
CIMGUI_API const ImGuiPayload *ImGui_GetDragDropPayload(
    void); // peek directly into the current payload from anywhere. may return
...
CIMGUI_API void ImGui_PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max,
...
CIMGUI_API void ImGui_SetItemDefaultFocus(
    void); // make last item the default focused item of a window.
CIMGUI_API void ImGui_SetKeyboardFocusHere(void); // Implied offset = 0
CIMGUI_API void ImGui_SetKeyboardFocusHereEx(
    int offset /* = 0 */); // focus keyboard on the next widget. Use positive
                           // 'offset' to access sub components of a multiple
                           // component widget. Use -1 to access previous
                           // widget.

// Overlapping mode
CIMGUI_API void ImGui_SetNextItemAllowOverlap(
    void); // allow next item to be overlapped by a subsequent item. Useful with
...
CIMGUI_API bool ImGui_IsItemHovered(
    ImGuiHoveredFlags
        flags /* = 0 */); // is the last item hovered? (and usable, aka not
                          // blocked by a popup, etc.). See ImGuiHoveredFlags
                          // for more options.
CIMGUI_API bool
ImGui_IsItemActive(void); // is the last item active? (e.g. button being held,
...
CIMGUI_API bool ImGui_IsItemFocused(
    void); // is the last item focused for keyboard/gamepad navigation?
CIMGUI_API bool ImGui_IsItemClicked(void); // Implied mouse_button = 0
CIMGUI_API bool ImGui_IsItemClickedEx(
    ImGuiMouseButton
        mouse_button /* = 0 */); // is the last item hovered and mouse clicked
                                 // on? (**)  == IsMouseClicked(mouse_button) &&
                                 // IsItemHovered()Important. (**) this is NOT
                                 // equivalent to the behavior of e.g. Button().
                                 // Read comments in function definition.
CIMGUI_API bool
ImGui_IsItemVisible(void); // is the last item visible? (items may be out of
                           // sight because of clipping/scrolling)
CIMGUI_API bool
ImGui_IsItemEdited(void); // did the last item modify its underlying value this
                          // frame? or was pressed? This is generally the same
                          // as the "bool" return value of many widgets.
CIMGUI_API bool ImGui_IsItemActivated(
    void); // was the last item just made active (item was previously inactive).
CIMGUI_API bool ImGui_IsItemDeactivated(
    void); // was the last item just made inactive (item was previously active).
           // Useful for Undo/Redo patterns with widgets that require continuous
           // editing.
CIMGUI_API bool ImGui_IsItemDeactivatedAfterEdit(
    void); // was the last item just made inactive and made a value change when
...
CIMGUI_API bool ImGui_IsItemToggledOpen(
    void); // was the last item open state toggled? set by TreeNode().
...
CIMGUI_API ImGuiID ImGui_GetItemID(
    void); // get ID of last item (~~ often same ImGui::GetID(label) beforehand)
CIMGUI_API ImVec2 ImGui_GetItemRectMin(
    void); // get upper-left bounding rectangle of the last item (screen space)
CIMGUI_API ImVec2 ImGui_GetItemRectMax(
    void); // get lower-right bounding rectangle of the last item (screen space)
...
CIMGUI_API ImGuiViewport *ImGui_GetMainViewport(
    void); // return primary/default viewport. This can never be NULL.

// Background/Foreground Draw Lists
CIMGUI_API ImDrawList *ImGui_GetBackgroundDrawList(
    void); // this draw list will be the first rendered one. Useful to quickly
           // draw shapes/text behind dear imgui contents.
CIMGUI_API ImDrawList *ImGui_GetForegroundDrawList(
    void); // this draw list will be the last rendered one. Useful to quickly
           // draw shapes/text over dear imgui contents.

// Miscellaneous Utilities
CIMGUI_API bool ImGui_IsRectVisibleBySize(
    ImVec2 size); // test if rectangle (of given size, starting from cursor
                  // position) is visible / not clipped.
CIMGUI_API bool ImGui_IsRectVisible(
    ImVec2 rect_min,
    ImVec2 rect_max); // test if rectangle (in screen space) is visible / not
                      // clipped. to perform coarse clipping on user's side.
CIMGUI_API double ImGui_GetTime(
    void); // get global imgui time. incremented by io.DeltaTime every frame.
CIMGUI_API int ImGui_GetFrameCount(
    void); // get global imgui frame count. incremented by 1 every frame.
CIMGUI_API ImDrawListSharedData *ImGui_GetDrawListSharedData(
    void); // you may use this when creating your own ImDrawList instances.
CIMGUI_API const char *
ImGui_GetStyleColorName(ImGuiCol idx); // get a string corresponding to the enum
                                       // value (for display, saving, etc.).
CIMGUI_API void ImGui_SetStateStorage(
    ImGuiStorage *
...
CIMGUI_API bool ImGui_BeginChildFrame(
    ImGuiID id, ImVec2 size,
    ImGuiWindowFlags
        flags /* = 0 */); // helper to create a child window / scrolling region
                          // that looks like a normal widget frame
CIMGUI_API void ImGui_EndChildFrame(
    void); // always call EndChildFrame() regardless of BeginChildFrame() return
           // values (which indicates a collapsed/clipped window)

// Text Utilities
CIMGUI_API ImVec2 ImGui_CalcTextSize(
    const char *text); // Implied text_end = NULL, hide_text_after_double_hash =
                       // false, wrap_width = -1.0f
CIMGUI_API ImVec2
ImGui_CalcTextSizeEx(const char *text, const char *text_end /* = NULL */,
...
CIMGUI_API void ImGui_ColorConvertRGBtoHSV(float r, float g, float b,
                                           float *out_h, float *out_s,
                                           float *out_v);
CIMGUI_API void ImGui_ColorConvertHSVtoRGB(float h, float s, float v,
...
CIMGUI_API bool ImGui_IsKeyPressedEx(
    ImGuiKey key,
    bool repeat /* = true */); // was key pressed (went from !Down to Down)? if
                               // repeat=true, uses io.KeyRepeatDelay /
                               // KeyRepeatRate
CIMGUI_API bool ImGui_IsKeyReleased(
    ImGuiKey key); // was key released (went from Down to !Down)?
CIMGUI_API int ImGui_GetKeyPressedAmount(
...
CIMGUI_API const char *ImGui_GetKeyName(
...
CIMGUI_API bool
ImGui_IsMouseDown(ImGuiMouseButton button); // is mouse button held?
CIMGUI_API bool
ImGui_IsMouseClicked(ImGuiMouseButton button); // Implied repeat = false
CIMGUI_API bool ImGui_IsMouseClickedEx(
    ImGuiMouseButton button,
    bool repeat /* = false */); // did mouse button clicked? (went from !Down to
                                // Down). Same as GetMouseClickedCount() == 1.
CIMGUI_API bool
ImGui_IsMouseReleased(ImGuiMouseButton button); // did mouse button released?
                                                // (went from Down to !Down)
CIMGUI_API bool ImGui_IsMouseDoubleClicked(
    ImGuiMouseButton
...
CIMGUI_API int ImGui_GetMouseClickedCount(
    ImGuiMouseButton button); // return the number of successive mouse-clicks at
                              // the time where a click happen (otherwise 0).
CIMGUI_API bool ImGui_IsMouseHoveringRect(ImVec2 r_min,
                                          ImVec2 r_max); // Implied clip = true
CIMGUI_API bool ImGui_IsMouseHoveringRectEx(
    ImVec2 r_min, ImVec2 r_max,
    bool clip /* = true */); // is mouse hovering given bounding rect (in screen
                             // space). clipped by current clipping settings,
                             // but disregarding of other consideration of
                             // focus/window ordering/popup-block.
CIMGUI_API bool ImGui_IsMousePosValid(
    const ImVec2
        *mouse_pos /* = NULL */); // by convention we use (-FLT_MAX,-FLT_MAX) to
                                  // denote that there is no mouse available
CIMGUI_API bool ImGui_IsAnyMouseDown(
    void); // [WILL OBSOLETE] is any mouse button held? This was designed for
           // backends, but prefer having backend maintain a mask of held mouse
           // buttons, because upcoming input queue system will make this
           // invalid.
CIMGUI_API ImVec2
ImGui_GetMousePos(void); // shortcut to ImGui::GetIO().MousePos provided by
                         // user, to be consistent with other calls
CIMGUI_API ImVec2 ImGui_GetMousePosOnOpeningCurrentPopup(
    void); // retrieve mouse position at the time of opening popup we have
           // BeginPopup() into (helper to avoid user backing that value
           // themselves)
CIMGUI_API bool ImGui_IsMouseDragging(
    ImGuiMouseButton button,
    float lock_threshold /* = -1.0f */); // is mouse dragging? (if
                                         // lock_threshold < -1.0f, uses
                                         // io.MouseDraggingThreshold)
CIMGUI_API ImVec2 ImGui_GetMouseDragDelta(
    ImGuiMouseButton button /* = 0 */,
    float lock_threshold /* = -1.0f */); // return the delta from the initial
                                         // clicking position while the mouse
                                         // button is pressed or was just
                                         // released. This is locked and return
                                         // 0.0f until the mouse moves past a
                                         // distance threshold at least once (if
                                         // lock_threshold < -1.0f, uses
                                         // io.MouseDraggingThreshold)
CIMGUI_API void ImGui_ResetMouseDragDelta(void); // Implied button = 0
CIMGUI_API void
ImGui_ResetMouseDragDeltaEx(ImGuiMouseButton button /* = 0 */); //
CIMGUI_API ImGuiMouseCursor ImGui_GetMouseCursor(
    void); // get desired mouse cursor shape. Important: reset in
           // ImGui::NewFrame(), this is updated during the frame. valid before
           // Render(). If you use software rendering by setting
           // io.MouseDrawCursor ImGui will render those for you
CIMGUI_API void ImGui_SetMouseCursor(
    ImGuiMouseCursor cursor_type); // set desired mouse cursor shape
CIMGUI_API void ImGui_SetNextFrameWantCaptureMouse(
    bool
...
// executables).
CIMGUI_API void ImGui_LoadIniSettingsFromDisk(
    const char
...
CIMGUI_API void ImGui_LoadIniSettingsFromMemory(
    const char *ini_data,
    size_t ini_size /* = 0 */); // call after CreateContext() and before the
                                // first call to NewFrame() to provide .ini data
                                // from your own data source.
CIMGUI_API void ImGui_SaveIniSettingsToDisk(
    const char
        *ini_filename); // this is automatically called (if io.IniFilename is
                        ...
                        // DestroyContext).
CIMGUI_API const char *ImGui_SaveIniSettingsToMemory(
    size_t
        *out_ini_size /* = NULL */); // return a zero-terminated string with the
                                     // .ini data which you can save by your own
                                     // mean. call when io.WantSaveIniSettings
                                     // is set, then save data by your own mean
                                     // and clear io.WantSaveIniSettings.

// Debug Utilities
CIMGUI_API void ImGui_DebugTextEncoding(const char *text);
CIMGUI_API bool ImGui_DebugCheckVersionAndDataLayout(
...
CIMGUI_API void ImGui_SetAllocatorFunctions(ImGuiMemAllocFunc alloc_func,
                                            ImGuiMemFreeFunc free_func,
                                            void *user_data /* = NULL */);
CIMGUI_API void ImGui_GetAllocatorFunctions(ImGuiMemAllocFunc *p_alloc_func,
...
CIMGUI_API void ImVector_Construct(
    void *vector); // Construct a zero-size ImVector<> (of any type). This is
                   // primarily useful when calling
                   // ImFontGlyphRangesBuilder_BuildRanges()
CIMGUI_API void ImVector_Destruct(
    void *vector); // Destruct an ImVector<> (of any type). Important: Frees the
                   // vector memory but does not call destructors on contained
                   // objects (if they have them)

#if defined(IMGUI_HAS_IMSTR)
#if IMGUI_HAS_IMSTR
CIMGUI_API ImStr ImStr_FromCharStr(
    const char *b); // Build an ImStr from a regular const char* (no data is
                    // copied, so you need to make sure the original char* isn't
                    // altered as long as you are using the ImStr).
...
// Input Functions
CIMGUI_API void ImGuiIO_AddKeyEvent(
    ImGuiIO *self, ImGuiKey key,
    bool down); // Queue a new key down/up event. Key should be "translated" (as
                // in, generally ImGuiKey_A matches the key end-user would use
                // to emit an 'A' character)
CIMGUI_API void ImGuiIO_AddKeyAnalogEvent(
    ImGuiIO *self, ImGuiKey key, bool down,
    float v); // Queue a new key down/up event for analog values (e.g.
              // ImGuiKey_Gamepad_ values). Dead-zones should be handled by the
              // backend.
CIMGUI_API void ImGuiIO_AddMousePosEvent(
    ImGuiIO *self, float x,
    float y); // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify
              // no mouse (e.g. app not focused and not hovered)
CIMGUI_API void
ImGuiIO_AddMouseButtonEvent(ImGuiIO *self, int button,
                            bool down); // Queue a mouse button change
CIMGUI_API void ImGuiIO_AddMouseWheelEvent(
    ImGuiIO *self, float wheel_x,
    float wheel_y); // Queue a mouse wheel update. wheel_y<0: scroll down,
                    // wheel_y>0: scroll up, wheel_x<0: scroll right, wheel_x>0:
                    // scroll left.
CIMGUI_API void ImGuiIO_AddMouseSourceEvent(
    ImGuiIO *self,
    ImGuiMouseSource
        source); // Queue a mouse source change (Mouse/TouchScreen/Pen)
CIMGUI_API void ImGuiIO_AddFocusEvent(
    ImGuiIO *self,
    bool focused); // Queue a gain/loss of focus for the application (generally
                   // based on OS/platform focus of your window)
CIMGUI_API void
ImGuiIO_AddInputCharacter(ImGuiIO *self,
                          unsigned int c); // Queue a new character input
CIMGUI_API void ImGuiIO_AddInputCharacterUTF16(
    ImGuiIO *self, ImWchar16 c); // Queue a new character input from a UTF-16
                                 // character, it can be a surrogate
CIMGUI_API void ImGuiIO_AddInputCharactersUTF8(
    ImGuiIO *self,
    const char *str); // Queue a new characters input from a UTF-8 string
CIMGUI_API void ImGuiIO_SetKeyEventNativeData(
    ImGuiIO *self, ImGuiKey key, int native_keycode,
    int native_scancode); // Implied native_legacy_index = -1
CIMGUI_API void ImGuiIO_SetKeyEventNativeDataEx(
    ImGuiIO *self, ImGuiKey key, int native_keycode, int native_scancode,
    int native_legacy_index /* = -1 */); // [Optional] Specify index for legacy
                                         // <1.87 IsKeyXXX() functions with
                                         // native indices + specify native
                                         // keycode, scancode.
CIMGUI_API void ImGuiIO_SetAppAcceptingEvents(
    ImGuiIO *self,
    bool accepting_events); // Set master flag for accepting key/mouse/text
                            ...
                            // application loop/refresh, and you want to disable
                            // events being queued while your app is frozen.
CIMGUI_API void
ImGuiIO_ClearEventsQueue(ImGuiIO *self); // Clear all incoming events.
CIMGUI_API void ImGuiIO_ClearInputKeys(
    ImGuiIO
        *self); // Clear current keyboard/mouse/gamepad state + current frame
                // text input buffer. Equivalent to releasing all keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
CIMGUI_API void ImGuiIO_ClearInputCharacters(
    ImGuiIO *self); // [Obsolete] Clear the current frame text input buffer. Now
...
CIMGUI_API void
ImGuiInputTextCallbackData_InsertChars(ImGuiInputTextCallbackData *self,
                                       int pos, const char *text,
                                       const char *text_end /* = NULL */);
CIMGUI_API void
ImGuiInputTextCallbackData_SelectAll(ImGuiInputTextCallbackData *self);
CIMGUI_API void
ImGuiInputTextCallbackData_ClearSelection(ImGuiInputTextCallbackData *self);
CIMGUI_API bool
ImGuiInputTextCallbackData_HasSelection(const ImGuiInputTextCallbackData *self);
...
CIMGUI_API bool ImGuiPayload_IsDataType(const ImGuiPayload *self,
                                        const char *type);
...
CIMGUI_API bool ImGuiTextFilter_ImGuiTextRange_empty(
    const ImGuiTextFilter_ImGuiTextRange *self);
CIMGUI_API void ImGuiTextFilter_ImGuiTextRange_split(
    const ImGuiTextFilter_ImGuiTextRange *self, char separator,
    ImVector_ImGuiTextFilter_ImGuiTextRange *out);
...
CIMGUI_API bool ImGuiTextFilter_Draw(
    ImGuiTextFilter *self, const char *label /* = "Filter (inc,-exc)" */,
    float width /* = 0.0f */); // Helper calling InputText+Build
CIMGUI_API bool ImGuiTextFilter_PassFilter(const ImGuiTextFilter *self,
                                           const char *text,
                                           const char *text_end /* = NULL */);
...
CIMGUI_API const char *ImGuiTextBuffer_end(
    const ImGuiTextBuffer *self); // Buf is zero-terminated, so end() will point
...
CIMGUI_API void ImGuiTextBuffer_append(ImGuiTextBuffer *self, const char *str,
                                       const char *str_end /* = NULL */);
CIMGUI_API void ImGuiTextBuffer_appendf(ImGuiTextBuffer *self, const char *fmt,
                                        ...) IM_FMTARGS(2);
CIMGUI_API void ImGuiTextBuffer_appendfv(ImGuiTextBuffer *self, const char *fmt,
                                         va_list args) IM_FMTLIST(2);
...
CIMGUI_API int ImGuiStorage_GetInt(const ImGuiStorage *self, ImGuiID key,
                                   int default_val /* = 0 */);
CIMGUI_API void ImGuiStorage_SetInt(ImGuiStorage *self, ImGuiID key, int val);
CIMGUI_API bool ImGuiStorage_GetBool(const ImGuiStorage *self, ImGuiID key,
                                     bool default_val /* = false */);
CIMGUI_API void ImGuiStorage_SetBool(ImGuiStorage *self, ImGuiID key, bool val);
CIMGUI_API float ImGuiStorage_GetFloat(const ImGuiStorage *self, ImGuiID key,
                                       float default_val /* = 0.0f */);
CIMGUI_API void ImGuiStorage_SetFloat(ImGuiStorage *self, ImGuiID key,
                                      float val);
CIMGUI_API void *ImGuiStorage_GetVoidPtr(const ImGuiStorage *self,
                                         ImGuiID key); // default_val is NULL
CIMGUI_API void ImGuiStorage_SetVoidPtr(ImGuiStorage *self, ImGuiID key,
                                        void *val);
...
CIMGUI_API int *ImGuiStorage_GetIntRef(ImGuiStorage *self, ImGuiID key,
                                       int default_val /* = 0 */);
CIMGUI_API bool *ImGuiStorage_GetBoolRef(ImGuiStorage *self, ImGuiID key,
                                         bool default_val /* = false */);
CIMGUI_API float *ImGuiStorage_GetFloatRef(ImGuiStorage *self, ImGuiID key,
                                           float default_val /* = 0.0f */);
CIMGUI_API void **ImGuiStorage_GetVoidPtrRef(ImGuiStorage *self, ImGuiID key,
                                             void *default_val /* = NULL */);
// Use on your own storage if you know only integer are being stored (open/close
// all tree nodes)
...
CIMGUI_API void ImGuiListClipper_Begin(ImGuiListClipper *self, int items_count,
                                       float items_height /* = -1.0f */);
CIMGUI_API void ImGuiListClipper_End(
    ImGuiListClipper *self); // Automatically called on the last call of Step()
                             // that returns false.
CIMGUI_API bool ImGuiListClipper_Step(
    ImGuiListClipper
        *self); // Call until it returns false. The DisplayStart/DisplayEnd
...
// visibility. (Due to alignment / padding of certain items it is possible that
...
CIMGUI_API void ImGuiListClipper_IncludeItemByIndex(ImGuiListClipper *self,
                                                    int item_index);
CIMGUI_API void ImGuiListClipper_IncludeItemsByIndex(
    ImGuiListClipper *self, int item_begin,
    int item_end); // item_end is exclusive e.g. use (42, 42+1) to make item 42
                   // never clipped.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
CIMGUI_API void
ImGuiListClipper_IncludeRangeByIndices(ImGuiListClipper *self, int item_begin,
                                       int item_end); // [renamed in 1.89.9]
CIMGUI_API void ImGuiListClipper_ForceDisplayRangeByIndices(
    ImGuiListClipper *self, int item_begin,
    int item_end); // [renamed in 1.89.6]
#endif             // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
CIMGUI_API void ImColor_SetHSV(ImColor *self, float h, float s, float v,
                               float a /* = 1.0f */);
CIMGUI_API ImColor ImColor_HSV(ImColor *self, float h, float s, float v,
                               float a /* = 1.0f */);
...
typedef struct ImDrawListSplitter_t {
  ...
  ImVector_ImDrawChannel _Channels; // Draw channels (not resized down so _Count
                                    // might be < Channels.Size)
} ImDrawListSplitter;
CIMGUI_API void ImDrawListSplitter_Clear(
    ImDrawListSplitter *self); // Do not clear Channels[] so our allocations are
                               // reused next frame
...
CIMGUI_API void ImDrawListSplitter_Split(ImDrawListSplitter *self,
                                         ImDrawList *draw_list, int count);
...
typedef struct ImDrawList_t {
  ...
  ImVector_ImDrawCmd CmdBuffer;  // Draw commands. Typically 1 command = 1 GPU
                                 // draw call, unless the command is a callback.
  ImVector_ImDrawIdx IdxBuffer;  // Index buffer. Each command consume
...
CIMGUI_API void ImDrawList_PushClipRect(
    ImDrawList *self, ImVec2 clip_rect_min, ImVec2 clip_rect_max,
    bool intersect_with_current_clip_rect /* = false */); // Render-level
                                                          // scissoring. This is
                                                          // passed down to your
                                                          // render function but
                                                          // not used for
...
//   In older versions (until Dear ImGui 1.77) the AddCircle functions defaulted
//   to num_segments == 12. In future versions we will use textures to provide
//   cheaper and higher-quality circles. Use AddNgon() and AddNgonFilled()
//   functions if you need to guarantee a specific number of sides.
CIMGUI_API void ImDrawList_AddLine(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                   ImU32 col); // Implied thickness = 1.0f
CIMGUI_API void ImDrawList_AddLineEx(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                     ImU32 col, float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_AddRect(
    ImDrawList *self, ImVec2 p_min, ImVec2 p_max,
    ImU32 col); // Implied rounding = 0.0f, flags = 0, thickness = 1.0f
CIMGUI_API void ImDrawList_AddRectEx(
    ImDrawList *self, ImVec2 p_min, ImVec2 p_max, ImU32 col,
    float rounding /* = 0.0f */, ImDrawFlags flags /* = 0 */,
    float thickness /* = 1.0f */); // a: upper-left, b: lower-right (==
                                   // upper-left + size)
CIMGUI_API void
ImDrawList_AddRectFilled(ImDrawList *self, ImVec2 p_min, ImVec2 p_max,
                         ImU32 col); // Implied rounding = 0.0f, flags = 0
CIMGUI_API void ImDrawList_AddRectFilledEx(
    ImDrawList *self, ImVec2 p_min, ImVec2 p_max, ImU32 col,
    float rounding /* = 0.0f */,
    ImDrawFlags flags /* = 0 */); // a: upper-left, b: lower-right (==
                                  // upper-left + size)
CIMGUI_API void
ImDrawList_AddRectFilledMultiColor(ImDrawList *self, ImVec2 p_min, ImVec2 p_max,
                                   ImU32 col_upr_left, ImU32 col_upr_right,
                                   ImU32 col_bot_right, ImU32 col_bot_left);
CIMGUI_API void ImDrawList_AddQuad(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                   ImVec2 p3, ImVec2 p4,
                                   ImU32 col); // Implied thickness = 1.0f
CIMGUI_API void ImDrawList_AddQuadEx(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                     ImVec2 p3, ImVec2 p4, ImU32 col,
                                     float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_AddQuadFilled(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                         ImVec2 p3, ImVec2 p4, ImU32 col);
CIMGUI_API void ImDrawList_AddTriangle(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                       ImVec2 p3,
                                       ImU32 col); // Implied thickness = 1.0f
CIMGUI_API void ImDrawList_AddTriangleEx(ImDrawList *self, ImVec2 p1, ImVec2 p2,
                                         ImVec2 p3, ImU32 col,
                                         float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_AddTriangleFilled(ImDrawList *self, ImVec2 p1,
                                             ImVec2 p2, ImVec2 p3, ImU32 col);
CIMGUI_API void
ImDrawList_AddCircle(ImDrawList *self, ImVec2 center, float radius,
                     ImU32 col); // Implied num_segments = 0, thickness = 1.0f
CIMGUI_API void ImDrawList_AddCircleEx(ImDrawList *self, ImVec2 center,
                                       float radius, ImU32 col,
                                       int num_segments /* = 0 */,
                                       float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_AddCircleFilled(ImDrawList *self, ImVec2 center,
                                           float radius, ImU32 col,
                                           int num_segments /* = 0 */);
CIMGUI_API void
ImDrawList_AddNgon(ImDrawList *self, ImVec2 center, float radius, ImU32 col,
                   int num_segments); // Implied thickness = 1.0f
CIMGUI_API void ImDrawList_AddNgonEx(ImDrawList *self, ImVec2 center,
                                     float radius, ImU32 col, int num_segments,
                                     float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_AddNgonFilled(ImDrawList *self, ImVec2 center,
                                         float radius, ImU32 col,
                                         int num_segments);
CIMGUI_API void
ImDrawList_AddText(ImDrawList *self, ImVec2 pos, ImU32 col,
                   const char *text_begin); // Implied text_end = NULL
CIMGUI_API void ImDrawList_AddTextEx(ImDrawList *self, ImVec2 pos, ImU32 col,
                                     const char *text_begin,
                                     const char *text_end /* = NULL */);
CIMGUI_API void ImDrawList_AddTextImFontPtr(
    ImDrawList *self, const ImFont *font, float font_size, ImVec2 pos,
    ImU32 col, const char *text_begin); // Implied text_end = NULL, wrap_width =
                                        // 0.0f, cpu_fine_clip_rect = NULL
CIMGUI_API void ImDrawList_AddTextImFontPtrEx(
    ImDrawList *self, const ImFont *font, float font_size, ImVec2 pos,
    ImU32 col, const char *text_begin, const char *text_end /* = NULL */,
    float wrap_width /* = 0.0f */,
    const ImVec4 *cpu_fine_clip_rect /* = NULL */);
CIMGUI_API void ImDrawList_AddPolyline(ImDrawList *self, const ImVec2 *points,
...
CIMGUI_API void ImDrawList_AddBezierCubic(
    ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImU32 col,
    float thickness,
    int num_segments /* = 0 */); // Cubic Bezier (4 control points)
CIMGUI_API void ImDrawList_AddBezierQuadratic(
    ImDrawList *self, ImVec2 p1, ImVec2 p2, ImVec2 p3, ImU32 col,
    float thickness,
    int num_segments /* = 0 */); // Quadratic Bezier (3 control points)
// Image primitives
// - Read FAQ to understand what ImTextureID is.
// - "p_min" and "p_max" represent the upper-left and lower-right corners of the
// rectangle.
...
CIMGUI_API void
ImDrawList_AddImage(ImDrawList *self, ImTextureID user_texture_id, ImVec2 p_min,
                    ImVec2 p_max); // Implied uv_min = ImVec2(0, 0), uv_max =
                                   // ImVec2(1, 1), col = IM_COL32_WHITE
CIMGUI_API void ImDrawList_AddImageEx(ImDrawList *self,
                                      ImTextureID user_texture_id, ImVec2 p_min,
                                      ImVec2 p_max,
                                      ImVec2 uv_min /* = ImVec2(0, 0) */,
                                      ImVec2 uv_max /* = ImVec2(1, 1) */,
                                      ImU32 col /* = IM_COL32_WHITE */);
CIMGUI_API void ImDrawList_AddImageQuad(
    ImDrawList *self, ImTextureID user_texture_id, ImVec2 p1, ImVec2 p2,
    ImVec2 p3,
    ImVec2 p4); // Implied uv1 = ImVec2(0, 0), uv2 = ImVec2(1, 0), uv3 =
                // ImVec2(1, 1), uv4 = ImVec2(0, 1), col = IM_COL32_WHITE
CIMGUI_API void ImDrawList_AddImageQuadEx(
    ImDrawList *self, ImTextureID user_texture_id, ImVec2 p1, ImVec2 p2,
    ImVec2 p3, ImVec2 p4, ImVec2 uv1 /* = ImVec2(0, 0) */,
    ImVec2 uv2 /* = ImVec2(1, 0) */, ImVec2 uv3 /* = ImVec2(1, 1) */,
    ImVec2 uv4 /* = ImVec2(0, 1) */, ImU32 col /* = IM_COL32_WHITE */);
...
CIMGUI_API void ImDrawList_PathStroke(ImDrawList *self, ImU32 col,
                                      ImDrawFlags flags /* = 0 */,
                                      float thickness /* = 1.0f */);
CIMGUI_API void ImDrawList_PathArcTo(ImDrawList *self, ImVec2 center,
                                     float radius, float a_min, float a_max,
                                     int num_segments /* = 0 */);
CIMGUI_API void ImDrawList_PathArcToFast(
    ImDrawList *self, ImVec2 center, float radius, int a_min_of_12,
    int a_max_of_12); // Use precomputed angles for a 12 steps circle
CIMGUI_API void ImDrawList_PathBezierCubicCurveTo(
    ImDrawList *self, ImVec2 p2, ImVec2 p3, ImVec2 p4,
    int num_segments /* = 0 */); // Cubic Bezier (4 control points)
CIMGUI_API void ImDrawList_PathBezierQuadraticCurveTo(
    ImDrawList *self, ImVec2 p2, ImVec2 p3,
    int num_segments /* = 0 */); // Quadratic Bezier (3 control points)
CIMGUI_API void ImDrawList_PathRect(ImDrawList *self, ImVec2 rect_min,
                                    ImVec2 rect_max,
                                    float rounding /* = 0.0f */,
                                    ImDrawFlags flags /* = 0 */);
// Advanced
CIMGUI_API void ImDrawList_AddCallback(
    ImDrawList *self, ImDrawCallback callback,
    void *callback_data); // Your rendering function must check for
                          // 'UserCallback' in ImDrawCmd and call the function
                          // instead of rendering triangles.
CIMGUI_API void ImDrawList_AddDrawCmd(
    ImDrawList *self); // This is useful if you need to forcefully create a new
...
CIMGUI_API ImDrawList *ImDrawList_CloneOutput(
    const ImDrawList
        *self); // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.
// Advanced: Channels
...
//   Prefer using your own persistent instance of ImDrawListSplitter as you can
//   stack them. Using the ImDrawList::ChannelsXXXX you cannot stack a split
//   over another.
...
CIMGUI_API void ImDrawList_PrimReserve(ImDrawList *self, int idx_count,
                                       int vtx_count);
CIMGUI_API void ImDrawList_PrimUnreserve(ImDrawList *self, int idx_count,
                                         int vtx_count);
CIMGUI_API void ImDrawList_PrimRect(
    ImDrawList *self, ImVec2 a, ImVec2 b,
    ImU32 col); // Axis aligned rectangle (composed of two triangles)
CIMGUI_API void ImDrawList_PrimRectUV(ImDrawList *self, ImVec2 a, ImVec2 b,
                                      ImVec2 uv_a, ImVec2 uv_b, ImU32 col);
CIMGUI_API void ImDrawList_PrimQuadUV(ImDrawList *self, ImVec2 a, ImVec2 b,
                                      ImVec2 c, ImVec2 d, ImVec2 uv_a,
                                      ImVec2 uv_b, ImVec2 uv_c, ImVec2 uv_d,
                                      ImU32 col);
CIMGUI_API void ImDrawList_PrimWriteVtx(ImDrawList *self, ImVec2 pos, ImVec2 uv,
                                        ImU32 col);
CIMGUI_API void ImDrawList_PrimWriteIdx(ImDrawList *self, ImDrawIdx idx);
CIMGUI_API void ImDrawList_PrimVtx(ImDrawList *self, ImVec2 pos, ImVec2 uv,
                                   ImU32 col); // Write vertex with unique index
// [Internal helpers]
...
CIMGUI_API int ImDrawList__CalcCircleAutoSegmentCount(const ImDrawList *self,
                                                      float radius);
CIMGUI_API void ImDrawList__PathArcToFastEx(ImDrawList *self, ImVec2 center,
                                            float radius, int a_min_sample,
                                            int a_max_sample, int a_step);
CIMGUI_API void ImDrawList__PathArcToN(ImDrawList *self, ImVec2 center,
                                       float radius, float a_min, float a_max,
                                       int num_segments);
...
CIMGUI_API void ImDrawData_AddDrawList(
    ImDrawData *self,
    ImDrawList *draw_list); // Helper to add an external draw list into an
                            // existing ImDrawData.
CIMGUI_API void ImDrawData_DeIndexAllBuffers(
    ImDrawData
        *self); // Helper to convert all buffers from indexed to non-indexed, in
                // case you cannot render indexed. Note: this is slow and most
                // likely a waste of resources. Always prefer indexed rendering!
CIMGUI_API void ImDrawData_ScaleClipRects(
    ImDrawData *self,
    ImVec2
        fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use
...
CIMGUI_API bool
ImFontGlyphRangesBuilder_GetBit(const ImFontGlyphRangesBuilder *self,
                                size_t n); // Get bit n in the array
CIMGUI_API void
ImFontGlyphRangesBuilder_SetBit(ImFontGlyphRangesBuilder *self,
                                size_t n); // Set bit n in the array
CIMGUI_API void ImFontGlyphRangesBuilder_AddChar(ImFontGlyphRangesBuilder *self,
                                                 ImWchar c); // Add character
CIMGUI_API void ImFontGlyphRangesBuilder_AddText(
    ImFontGlyphRangesBuilder *self, const char *text,
...
CIMGUI_API void ImFontGlyphRangesBuilder_BuildRanges(
    ImFontGlyphRangesBuilder *self,
    ImVector_ImWchar
        *out_ranges); // Output new ranges
                      // (ImVector_Construct()/ImVector_Destruct() can be used
                      // to safely construct out_ranges)
...
CIMGUI_API bool
ImFontAtlasCustomRect_IsPacked(const ImFontAtlasCustomRect *self);
...
CIMGUI_API ImFont *
ImFontAtlas_AddFontDefault(ImFontAtlas *self,
                           const ImFontConfig *font_cfg /* = NULL */);
CIMGUI_API ImFont *
ImFontAtlas_AddFontFromFileTTF(ImFontAtlas *self, const char *filename,
                               float size_pixels,
                               const ImFontConfig *font_cfg /* = NULL */,
                               const ImWchar *glyph_ranges /* = NULL */);
CIMGUI_API ImFont *ImFontAtlas_AddFontFromMemoryTTF(
    ImFontAtlas *self, void *font_data, int font_size, float size_pixels,
    const ImFontConfig *font_cfg /* = NULL */,
    const ImWchar
        *glyph_ranges /* = NULL */); // Note: Transfer ownership of 'ttf_data'
                                     // to ImFontAtlas! Will be deleted after
                                     // destruction of the atlas. Set
                                     // font_cfg->FontDataOwnedByAtlas=false to
                                     // keep ownership of your data and it won't
                                     // be freed.
CIMGUI_API ImFont *ImFontAtlas_AddFontFromMemoryCompressedTTF(
    ImFontAtlas *self, const void *compressed_font_data,
...
CIMGUI_API ImFont *ImFontAtlas_AddFontFromMemoryCompressedBase85TTF(
    ImFontAtlas *self, const char *compressed_font_data_base85,
    float size_pixels, const ImFontConfig *font_cfg /* = NULL */,
    const ImWchar
        *glyph_ranges /* = NULL */); // 'compressed_font_data_base85' still
                                     // owned by caller. Compress with
                                     // binary_to_compressed_c.cpp with -base85
                                     // parameter.
CIMGUI_API void ImFontAtlas_ClearInputData(
    ImFontAtlas *self); // Clear input data (all ImFontConfig structures
                        // including sizes, TTF data, glyph ranges, etc.) = all
                        // the data used to build the texture and fonts.
CIMGUI_API void ImFontAtlas_ClearTexData(
    ImFontAtlas *self); // Clear output texture data (CPU side). Saves RAM once
...
CIMGUI_API void
ImFontAtlas_Clear(ImFontAtlas *self); // Clear all input and output.
...
// Building in RGBA32 format is provided for convenience and compatibility, but
// note that unless you manually manipulate or copy color data into the texture
// (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will
// always be white (~75% of memory/bandwidth waste.
CIMGUI_API bool ImFontAtlas_Build(
    ImFontAtlas *self); // Build pixels data. This is called automatically for
                        // you by the GetTexData*** functions.
CIMGUI_API void ImFontAtlas_GetTexDataAsAlpha8(
    ImFontAtlas *self, unsigned char **out_pixels, int *out_width,
    int *out_height, int *out_bytes_per_pixel /* = NULL */); // 1 byte per-pixel
CIMGUI_API void ImFontAtlas_GetTexDataAsRGBA32(
    ImFontAtlas *self, unsigned char **out_pixels, int *out_width,
    int *out_height,
    int *out_bytes_per_pixel /* = NULL */); // 4 bytes-per-pixel
CIMGUI_API bool ImFontAtlas_IsBuilt(
    const ImFontAtlas
        *self); // Bit ambiguous: used to detect when user didn't build texture
...
// Helpers to retrieve list of common Unicode ranges (2 value per range, values
// are inclusive, zero-terminated list) NB: Make sure that your string are UTF-8
// and NOT in your local code page. Read
...
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesDefault(
    ImFontAtlas *self); // Basic Latin, Extended Latin
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesGreek(
    ImFontAtlas *self); // Default + Greek and Coptic
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesKorean(
    ImFontAtlas *self); // Default + Korean characters
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesJapanese(
    ImFontAtlas *self); // Default + Hiragana, Katakana, Half-Width, Selection
                        // of 2999 Ideographs
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesChineseFull(
    ImFontAtlas *self); // Default + Half-Width + Japanese Hiragana/Katakana +
                        // full set of about 21000 CJK Unified Ideographs
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon(
    ImFontAtlas
        *self); // Default + Half-Width + Japanese Hiragana/Katakana + set of
                // 2500 CJK Unified Ideographs for common simplified Chinese
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesCyrillic(
    ImFontAtlas *self); // Default + about 400 Cyrillic characters
CIMGUI_API const ImWchar *
ImFontAtlas_GetGlyphRangesThai(ImFontAtlas *self); // Default + Thai characters
CIMGUI_API const ImWchar *ImFontAtlas_GetGlyphRangesVietnamese(
    ImFontAtlas *self); // Default + Vietnamese characters
// You can request arbitrary rectangles to be packed into the atlas, for your
// own purposes.
// - After calling Build(), you can query the rectangle position and render your
// pixels.
...
CIMGUI_API int ImFontAtlas_AddCustomRectRegular(ImFontAtlas *self, int width,
                                                int height);
CIMGUI_API int
ImFontAtlas_AddCustomRectFontGlyph(ImFontAtlas *self, ImFont *font, ImWchar id,
                                   int width, int height, float advance_x,
                                   ImVec2 offset /* = ImVec2(0, 0) */);
CIMGUI_API ImFontAtlasCustomRect *
ImFontAtlas_GetCustomRectByIndex(ImFontAtlas *self, int index);
// [Internal]
CIMGUI_API void ImFontAtlas_CalcCustomRectUV(const ImFontAtlas *self,
                                             const ImFontAtlasCustomRect *rect,
                                             ImVec2 *out_uv_min,
                                             ImVec2 *out_uv_max);
CIMGUI_API bool ImFontAtlas_GetMouseCursorTexData(
    ImFontAtlas *self, ImGuiMouseCursor cursor, ImVec2 *out_offset,
...
typedef struct ImFont_t {
  ...
  ImVector_ImWchar IndexLookup; // 12-16 // out //            // Sparse. Index
...
CIMGUI_API const ImFontGlyph *ImFont_FindGlyphNoFallback(const ImFont *self,
                                                         ImWchar c);
...
// 'max_width' stops rendering after a certain width (could be turned into a 2d
// size). FLT_MAX to disable. 'wrap_width' enable automatic word-wrapping across
// multiple lines to fit into given width. 0.0f to disable.
CIMGUI_API ImVec2 ImFont_CalcTextSizeA(
    const ImFont *self, float size, float max_width, float wrap_width,
    const char *text_begin); // Implied text_end = NULL, remaining = NULL
CIMGUI_API ImVec2 ImFont_CalcTextSizeAEx(
    const ImFont *self, float size, float max_width, float wrap_width,
    const char *text_begin, const char *text_end /* = NULL */,
    const char **remaining /* = NULL */); // utf8
CIMGUI_API const char *
ImFont_CalcWordWrapPositionA(const ImFont *self, float scale, const char *text,
                             const char *text_end, float wrap_width);
CIMGUI_API void ImFont_RenderChar(const ImFont *self, ImDrawList *draw_list,
                                  float size, ImVec2 pos, ImU32 col, ImWchar c);
CIMGUI_API void ImFont_RenderText(const ImFont *self, ImDrawList *draw_list,
                                  float size, ImVec2 pos, ImU32 col,
                                  ImVec4 clip_rect, const char *text_begin,
                                  const char *text_end,
                                  float wrap_width /* = 0.0f */,
...
CIMGUI_API void ImFont_AddGlyph(ImFont *self, const ImFontConfig *src_cfg,
                                ImWchar c, float x0, float y0, float x1,
                                float y1, float u0, float v0, float u1,
                                float v1, float advance_x);
CIMGUI_API void ImFont_AddRemapChar(
    ImFont *self, ImWchar dst, ImWchar src,
    bool overwrite_dst /* = true */); // Makes 'dst' character/glyph points to
                                      // 'src' character/glyph. Currently needs
                                      // to be called AFTER fonts have been
                                      // built.
CIMGUI_API void ImFont_SetGlyphVisible(ImFont *self, ImWchar c, bool visible);
CIMGUI_API bool ImFont_IsGlyphRangeUnused(ImFont *self, unsigned int c_begin,
                                          unsigned int c_last);
...
CIMGUI_API void
ImGui_SetItemAllowOverlap(void); // Use SetNextItemAllowOverlap() before item.
// OBSOLETED in 1.89.4 (from March 2023)
CIMGUI_API void ImGui_PushAllowKeyboardFocus(bool tab_stop);
CIMGUI_API void ImGui_PopAllowKeyboardFocus(void);
// OBSOLETED in 1.89 (from August 2022)
CIMGUI_API bool ImGui_ImageButtonImTextureID(
    ImTextureID user_texture_id, ImVec2 size, ImVec2 uv0 /* = ImVec2(0, 0) */,
    ImVec2 uv1 /* = ImVec2(1, 1) */, int frame_padding /* = -1 */,
    ImVec4 bg_col /* = ImVec4(0, 0, 0, 0) */,
    ImVec4 tint_col /* = ImVec4(1, 1, 1, 1) */); // Use new ImageButton()
                                                 // signature (explicit item id,
                                                 // regular FramePadding)
// OBSOLETED in 1.88 (from May 2022)
CIMGUI_API void ImGui_CaptureKeyboardFromApp(
    bool want_capture_keyboard /* = true */); // Renamed as name was misleading
                                              // + removed default value.
CIMGUI_API void ImGui_CaptureMouseFromApp(
    bool want_capture_mouse /* = true */); // Renamed as name was misleading +
                                           // removed default value.
// OBSOLETED in 1.86 (from November 2021)
CIMGUI_API void ImGui_CalcListClipping(
...
#endif // #if defined(IMGUI_DISABLE_METRICS_WINDOW)&&
       // defined(IMGUI_DISABLE_OBSOLETE_FUNCTIONS)
//-----------------------------------------------------------------------------
