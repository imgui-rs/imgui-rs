...
impl GamepadHandler {
    fn new() -> GamepadHandler {
        GamepadHandler {
            connected_controllers: HashMap::new(),
        }
    }

    fn handle_event(&mut self, io: &mut Io, controller_event: &gilrs::Event) {
...
impl WinitPlatform {
    ...
    pub fn handle_controller_event(
        ...
        io: &mut Io,
        window: &Window,
        event: &Event<gilrs::Event>,
    ) {
        match event {
            Event::UserEvent(controller_event) => {
                self.controller.handle_event(io, controller_event)
            }
            _ => self.handle_event(io, window, event),
        }
    }
