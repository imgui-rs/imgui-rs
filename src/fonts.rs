...
impl FontGlyphRange {
    ...
    pub fn default() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Default) }

    /// A set of glyph ranges appropriate for use with Chinese text.
    pub fn chinese() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Chinese) }
    /// A set of glyph ranges appropriate for use with Cyrillic text.
    pub fn cyrillic() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Cyrillic) }
    /// A set of glyph ranges appropriate for use with Japanese text.
    pub fn japanese() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Japanese) }
    /// A set of glyph ranges appropriate for use with Korean text.
    pub fn korean() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Korean) }
    /// A set of glyph ranges appropriate for use with Thai text.
    pub fn thai() -> FontGlyphRange { FontGlyphRange(FontGlyphRangeData::Thai) }
...
impl ImFontConfig {
    ...
    fn make_config(self) -> sys::ImFontConfig {
        let mut config = unsafe {
            ...
            sys::ImFontConfig_ImFontConfig(&mut config);
            config
        };
...
impl Default for ImFontConfig {
    fn default() -> Self { ImFontConfig::new() }
}
...
impl<'a> ImFont<'a> {
    ...
    pub fn font_size(&self) -> f32 { unsafe { (*self.font).FontSize } }
    ...
    pub fn scale(&self) -> f32 { unsafe { (*self.font).Scale } }
    ...
    pub fn display_offset(&self) -> (f32, f32) {
        unsafe { (*self.font).DisplayOffset.clone().into() }
    }
}
...
impl<'a> ImFontAtlas<'a> {
    ...
    pub fn font_count(&self) -> usize { unsafe { (*self.atlas).Fonts.Size as _ } }
    ...
    pub fn index_font(&mut self, index: usize) -> ImFont {
        ...
        unsafe {
            let font = (*self.atlas).Fonts.Data.offset(index as _);
            ImFont::from_ptr(*font)
        }
    }

    /// Clears all fonts associated with this texture atlas.
    pub fn clear(&mut self) { unsafe { sys::ImFontAtlas_Clear(self.atlas) } }

    pub fn texture_id(&self) -> usize { unsafe { (*self.atlas).TexID as usize } }
